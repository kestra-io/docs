...
4. In the **Select trusted entity**, choose **Custom trust policy** and paste the following `Trust policy JSON:
    ...
    ```
   ![iam](@assets/docs/concepts/taskrunner-aws-batch/iam.png)
...
![create_role](@assets/docs/concepts/taskrunner-aws-batch/create_role.png)

Make sure to copy the ARN of the role. You will need it later.

![role_arn](@assets/docs/concepts/taskrunner-aws-batch/role_arn.png)
...
3. Select **JSON** and paste the following into the `Policy editor`:
    ...
    ```
   ![policy1](@assets/docs/concepts/taskrunner-aws-batch/policy1.png)
4. Select **Next** and type in a name for the policy, such as `ecsTaskRoleS3Policy`.
5. Once you're done, select **Create policy**.

![policy2](@assets/docs/concepts/taskrunner-aws-batch/policy2.png)
...
6. Search for the new policy and check the box on the left. Once you've done this, select **Next**.
   ![role_permission](@assets/docs/concepts/taskrunner-aws-batch/role_permission.png)
...
![batch4_search](@assets/docs/concepts/taskrunner-aws-batch/batch4_search.png)

Then, click on "Get Started". If you don't see the "Get Started" button, add `#firstRun` to the URL:

![batch4_firstrun](@assets/docs/concepts/taskrunner-aws-batch/batch4_firstrun.png)

This will launch a Wizard that will guide you through the process of creating a new compute environment.

![batch4_jobtype](@assets/docs/concepts/taskrunner-aws-batch/batch4_jobtype.png)
...
![batch5](@assets/docs/concepts/taskrunner-aws-batch/batch5.png)
...
![batch6](@assets/docs/concepts/taskrunner-aws-batch/batch6.png)
...
![batch7](@assets/docs/concepts/taskrunner-aws-batch/batch7.png)

#### Step 5: Create a job

Finally, create a job. Here, we name it `kestra`. Then, click on Next for a final review:

![batch8](@assets/docs/concepts/taskrunner-aws-batch/batch8.png)

#### Step 6: Review and create

Review your settings and click on Create resources:

![batch9](@assets/docs/concepts/taskrunner-aws-batch/batch9.png)

Once you see this message, you are all set:

![batch10](@assets/docs/concepts/taskrunner-aws-batch/batch10.png)
...
![batch11](@assets/docs/concepts/taskrunner-aws-batch/batch11.png)

![batch12](@assets/docs/concepts/taskrunner-aws-batch/batch12.png)

### Create an S3 Bucket

Last thing we'll need is an S3 storage bucket. To do this, select S3 from Services then select **Create bucket**.

![s3_create](@assets/docs/concepts/taskrunner-aws-batch/s3_create.png)

Next you'll need to add a name and leave everything else as a default value.

![s3_bucket_name](@assets/docs/concepts/taskrunner-aws-batch/s3_bucket_name.png)
...
![logs](@assets/docs/concepts/taskrunner-aws-batch/logs.png)
