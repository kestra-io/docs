...
### Go Production
![Kestra user interface](./architecture.jpg)
...
The usage grew exponentially over the coming months, and the kinds of usage are becoming increasingly varied: (starting flows based on [file detection from buckets](/plugins/plugin-gcp/google-cloud-storage-gcs/io.kestra.plugin.gcp.gcs.trigger), syncing data warehouses directly from operation [postgres database](/plugins/plugin-jdbc-postgres/io.kestra.plugin.jdbc.postgresql.copyout), developing a simple python program to [fetch APIs from a partner](/plugins/plugin-script-python/io.kestra.plugin.scripts.python.script), starting long-running data science processes [over Kubernetes](/plugins/plugin-kubernetes/kubernetes-core/io.kestra.plugin.kubernetes.core.podcreate), [fetching data from Google Drive](/plugins/plugin-googleworkspace/google-sheets/io.kestra.plugin.googleworkspace.sheets.read) and writing results back to [Google Sheets](/plugins/plugin-googleworkspace/google-drive/io.kestra.plugin.googleworkspace.drive.create), and so on.).
...
![Kestra user interface](./executions.svg)
![Kestra user interface](./taskruns.svg)
![Kestra user interface](./execution-duration.svg)
