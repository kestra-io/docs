...
There is an `artifact-sdk` repository that acts as a development studio for building and publishing them, and `plugin-gcp` is the reference implementation. The roadmap targets the places the payoff is largest: AI tasks, task runners starting with Kubernetes, dbt, Ansible and Terraform.
