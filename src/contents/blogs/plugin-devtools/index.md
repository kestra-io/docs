...
`kestra-plugins-search` runs a ripgrep search across every clone in parallel and groups matches by repo, respecting `.gitignore` and defaulting to Java files. Finding every implementer of an interface across the fleet becomes one command:
...
```bash
kestra-plugins-merge-dependabot-prs --dry-run
kestra-plugins-merge-dependabot-prs
```
