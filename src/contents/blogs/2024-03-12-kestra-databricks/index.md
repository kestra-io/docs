...
![top_10_orders_query_output](./top_10_orders_query_output.png)

### Delete Compute Cluster ###

Now that we have performed all the tasks on the Databricks cluster, we can delete it to avoid unnecessary compute costs. Kestra’s Databricks plugin provides the [DeleteCluster task](/plugins/plugin-databricks/databricks-cluster/io.kestra.plugin.databricks.cluster.deletecluster)  that will delete the compute cluster. This task takes the `clusterId` to pick the corresponding cluster for deletion.
