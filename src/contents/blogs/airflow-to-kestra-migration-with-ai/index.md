...
Airflow 2 recently reached end of life. For most teams, the easiest path is to upgrade to [Airflow 3](../airflow-3-vs-airflow-2/index.md) and move on. I think that's a mistake worth pausing on. The Airflow 3 [open issue tracker](https://github.com/apache/airflow/issues?q=is%3Aissue+is%3Aopen+airflow+3+upgrade) hints at a non-trivial migration even within the ecosystem, and the end of a major version is one of the few moments where the switching cost is low enough to seriously evaluate alternatives. If you're still weighing that decision, the [Airflow 2 end-of-life guide](/resources/airflow-2-eol-whitepaper) covers the upgrade-or-rethink question in depth.
...
LLMs have started to change that calculus. I built [migration-skills](https://github.com/kestra-io/agent-skills/blob/main/skills/migrate-airflow-kestra/SKILL.md) so the conversion part takes minutes instead of weeks. Convert a DAG, run it in Kestra, and you have something to show the right people the same day you decide to look.
