...
Namespaces are there from the first workflow you write. [Get started in minutes](../../docs/02.installation/index.mdx) with the open source edition on Docker to see how they organize a project. Tenants and worker groups are what you add on top when a deployment needs hard isolation between business units or execution split across regions, if you're scoping that kind of deployment, [book a demo](/demo) and we can work through the design together.
