...
- **Helm**: `Upgrade`, `Rollback`, `Status`, and `Uninstall` emit a `Custom` output typed `io.kestra.plugin.helm.assets.Release` for the release plus one `Custom` output typed `io.kestra.plugin.helm.assets.KubernetesResource` per managed resource (`Deployment`, `Service`, `Ingress`, etc.) — there are no built-in `HelmRelease`/`KubernetesResource` asset classes yet, so these use the plugin's own type strings; the chart reference and any `valuesFrom` files are declared as inputs.
