...
States come from the service lifecycle. Typical transitions are:
...
stateDiagram-v2
    ...
    DISCONNECTED --> TERMINATING
    DISCONNECTED --> NOT_RUNNING

    TERMINATING --> TERMINATED_GRACEFULLY
    TERMINATING --> TERMINATED_FORCED
    TERMINATING --> NOT_RUNNING

    TERMINATED_GRACEFULLY --> NOT_RUNNING
    TERMINATED_FORCED --> NOT_RUNNING
...
- **DISCONNECTED** signals a lost dependency (DB/queue). It either recovers to **RUNNING** or proceeds to shutdown.
