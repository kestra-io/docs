...
Navigate to **Super Admin → Infrastructure → Worker Queues** and click **Create**. You can also create Worker Queues via the API or Terraform.
...
Navigate to **Super Admin → Infrastructure → Worker Groups** and click **Add Worker Group**. Set an id, display name, and optional description. You can add queue subscriptions and generate registration tokens immediately, or configure them after creation.
