---
title: Kestra Expressions Full Reference – Pebble and Context Variables
...
Kestra's expressions combine the [Pebble templating engine](../../06.concepts/06.pebble/index.md) with the execution context to dynamically render flow properties. This page contains the complete legacy reference for expressions, filters, functions, operators, tags, and tests.
...
<div class="video-container">
  <iframe width="560" height="315" src="https://www.youtube.com/embed/SPGmXSJN3VE?si=rCoWNDEq14LYvPdM" title="YouTube video player" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
</div>
...
Kestra provides access to environment variables prefixed with `ENV_` by default, unless configured otherwise in the `variables` [runtime and storage configuration](../../configuration/02.runtime-and-storage/index.md).
...
To reference the [environment name](../../configuration/02.runtime-and-storage/index.md) defined in Kestra configuration, use `{{ kestra.environment }}`. To reference the environment URL, use `{{ kestra.url }}` from your Kestra configuration.

### Global Variables

You can define global variables in Kestra's [runtime and storage configuration](../../configuration/02.runtime-and-storage/index.md) and access them using `{{ globals.foo }}`.
...
Secrets are supported in both the open-source version and [Enterprise Edition](../../oss-vs-paid/index.md). For additional details, refer to the [Secrets](../../06.concepts/04.secret/index.md) documentation.

### Credentials (EE)

In **Enterprise Edition**, use the `credential()` function to fetch the current access token generated by a [Credential](../../07.enterprise/03.auth/credentials/index.md):

```yaml
id: call_with_credential
namespace: company.team
...
`credential()` returns the short-lived token only (no client secrets). Define and manage the credential itself in the Kestra UI; the expression simply injects the active token at runtime.
...
To escape expressions or control structures, use the [`raw` tag](#raw). This prevents Pebble from interpreting content within `{{ ... }}` or `{% ... %}`.
...
:::alert{type="info"}
In versions prior to [v0.18.0](../../../blogs/2024-08-06-release-0-18/index.md), this filter was named `json`. Using `json` will still work but raises a warning in the UI.
:::
...
For additional examples that access partial JSON bodies using `jq`, see the [JSON How-to guide](../../15.how-to-guides/json/index.md).
...
Builds a link to a Kestra [App](../../07.enterprise/04.scalability/apps/index.md). Without arguments, it returns the App link for the current flow; provide an `appId` to target a specific App.
...
For details, see the [Handling null and undefined values](../../15.how-to-guides/null-values/index.md) guide.
