...
Content that must stay identical across several pages — edition notices, install commands, shared warnings — is single-sourced in the snippet library at `src/contents/docs/_snippets/`. Never copy such a block from one page to another; include it instead.

Include a Markdown snippet with a `::snippet` directive:

```markdown
::snippet{name="worker-groups-cloud"}
```

Include a shared code file as a code block with the `file=` meta:
...
Both are expanded at build time, and a reference to a missing snippet fails the build. Before creating a new snippet or editing an existing one, read the [content reuse strategy](https://github.com/kestra-io/docs/blob/main/CONTENT_REUSE.md) — it defines when a snippet is appropriate, the authoring rules (site-absolute links, no page-specific context), and the catalog you must keep up to date.
