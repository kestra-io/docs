...
<CodeTabs client:load storageKey="kestra-cli-preference" tabs={[
  { label: "kestractl", code: "kestractl flows export --namespace company.team --output-file company-team-flows.zip\nunzip company-team-flows.zip -d ./flows/" },
  { label: "curl", code: 'curl -u "admin@kestra.io:yourpassword" \\\n  "http://localhost:8080/api/v1/flows/export/by-namespace?namespace=company.team" \\\n  -o company-team-flows.zip\n\nunzip company-team-flows.zip -d ./flows/' }
]} />
...
`⚠ ✗` warnings have two severities. **v2-incompatible** (red `✗`) means Kestra 2.0 rejects the flow on save and it cannot be deployed at all. **Advisory** (yellow `⚠`) means the flow deploys but breaks at runtime (for example, `read()` with a removed `version=` argument). Each entry includes a `↳ docs:` line pointing to the relevant migration guide page.
...
| `io.kestra.plugin.core.flow.EachSequential` | Rewrite as `Loop`; same expression changes as `ForEach` | [ForEach and ForEachItem replaced by Loop](../foreach-loop/index.md) |
| `io.kestra.plugin.core.flow.EachParallel` | Rewrite as `Loop` with `concurrencyLimit`; same expression changes | [ForEach and ForEachItem replaced by Loop](../foreach-loop/index.md) |
...
### Bulk deployments with incompatible flows

If you are deploying a large namespace where some flows still need manual rewrites, use `--disable-v2-incompatible` instead of fixing everything first. The flag rewrites each incompatible flow into a disabled placeholder tagged `v2-migration: needs-manual-rewrite`, with the migrated definition preserved as comments. The placeholder deploys without error, so a single `kestractl flows deploy` can land the entire namespace.

Compatible flows run immediately. Incompatible ones are visible in the UI, searchable by label, and ready for manual review. Re-running the tool over its own output is a no-op: a disabled placeholder has no live v2-incompatible construct, so it produces no warning and is written through unchanged.

```bash
kestra-migrate --disable-v2-incompatible -o ./v2-flows/ ./flows/
```

This flag cannot be combined with `--stay-v1-compatible`.
...
```bash
grep -rn "plugin.core.flow.ForEach\|plugin.core.flow.EachSequential\|plugin.core.flow.EachParallel" ./v2-flows/
```

All four flow-iteration task types (`ForEach`, `ForEachItem`, `EachSequential`, and `EachParallel`) are removed in 2.0 and replaced by `Loop`. `kestra-migrate` flags each with `⚠ ✗` and writes the files through unchanged — the grep confirms which flows still need rewriting after Step 3.
...
| Tasks that call Kestra API internally (git sync, push, fetch tasks) | Add an `auth` block with credentials, or configure a namespace default service account — without it the task fails with `401 Unauthorized` | [SDK authentication](../sdk-authentication/index.md) |
...
<CodeTabs client:load storageKey="kestra-cli-preference" tabs={[
  { label: "kestractl", code: "kestractl flows deploy ./v2-flows/ --namespace company.team --override" },
  { label: "curl", code: 'zip -r v2-flows.zip ./v2-flows/\ncurl -u "admin@kestra.io:yourpassword" \\\n  -X POST "http://localhost:8080/api/v1/flows/import" \\\n  -F "fileUpload=@v2-flows.zip"' }
]} />
...
`kestra-migrate` flags `ForEach`, `ForEachItem`, `EachSequential`, `EachParallel`, `pluginDefaults`, and `taskDefaults` as `⚠ ✗` and writes those files through unchanged. They will fail on the 2.0 server. Confirm none remain in your output directory:
