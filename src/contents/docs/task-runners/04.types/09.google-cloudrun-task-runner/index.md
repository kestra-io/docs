...
:::alert{type="info"}
The task-level `logToFile` property controls where Kestra stores log lines it has already retrieved from Cloud Logging — it does not affect polling frequency or quota consumption. All log retrieval still goes through `entries.list` at the `completionCheckInterval` rate. To reduce Cloud Logging read-quota pressure, raise `completionCheckInterval`.
:::
