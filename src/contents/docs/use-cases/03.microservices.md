...
Kestra can:
- [Trigger](../05.workflow-components/07.triggers/index.md) your microservices from any event, schedule, or flow dependency
- Pass data of any size between services thanks to built-in [internal storage](../08.architecture/data-components.md#internal-storage)
...
3. **Add Triggers** – Use scheduled or event-based [triggers](../05.workflow-components/07.triggers/index.md) to start microservice workflows.
4. **Observe and Manage** – Use [Kestra’s UI](../09.ui/index.md) to monitor states, logs, and metrics. Rerun failed workflow executions or roll back with one click.
...
- [Explore Task Runners](../07.enterprise/04.scalability/task-runners.md) for scaling custom code and containerized services.
