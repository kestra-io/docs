...
Triggers are enabled or disabled from the **Triggers** tab of the flow editor. See [Disabled](../../16.disabled/index.md) for details.

If a trigger is locked due to an execution in progress, the **Unlock trigger** button is available in the Triggers tab. Manually unlocking a trigger may result in duplicate executions.
...
When a trigger is misconfigured, no executions are created. Trigger logs are accessible from the **Triggers** tab on the flow's page; click the trigger row to expand its logs.

The example flow below has a misconfigured `http_poll` trigger pointing to a non-existent host:
...
The Triggers tab shows a detailed error message identifying the problem:
...
Schedule trigger with `stopAfter: [FAILED]`:
...
HTTP trigger with `stopAfter: [SUCCESS]`:
...
The `inputs` property on any trigger sets input values before execution. Schedule trigger setting the `user` input:
