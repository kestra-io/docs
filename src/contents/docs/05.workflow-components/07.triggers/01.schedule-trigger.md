...
For example, set this up as a [System Flow](../../06.concepts/system-flows.md) and send an alert if any Schedule Triggers come back showing an issue:
...
By default, the trigger checks all schedules in the current Tenant ([Multi-tenancy](../../07.enterprise/02.governance/tenants.md) is an Enterprise feature) if no Namespace or Flow is specified.
