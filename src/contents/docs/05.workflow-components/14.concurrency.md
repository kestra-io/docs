...
Any [Trigger](../05.workflow-components/07.triggers/index.md) type supports concurrent executions, allowing multiple instances of the same workflow to run simultaneously. This enables more flexible and scalable workflow patterns.
...
In this example, even though the [Schedule trigger](../05.workflow-components/07.triggers/01.schedule-trigger.md) fires every second, only one execution will run at a time. Setting `allowConcurrent: true` would allow multiple executions to run simultaneously.
