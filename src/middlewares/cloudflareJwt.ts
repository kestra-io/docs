...
const fetch = async (request: Request, jwtAssertionToken: string | null): Promise<Response | null> => {
    ...
    try {
        ...
        const debug = JSON.stringify({CLOUDFLARE_TEAM_DOMAIN, CLOUDFLARE_POLICY_AUD, jwtAssertionToken});
...
export default defineMiddleware(async (context, next) => {
    ...
    const response = await fetch(context.request, context.request.headers.get("cf-access-jwt-assertion"));
