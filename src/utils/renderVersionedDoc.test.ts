...
describe("renderVersionedDocBody data-driven components", () => {
    ...
    it("nests a child's own children one level under it, mirroring ChildTableOfContents.astro", async () => {
        const withGrandchildren = {
            ...children,
            "docs/use-cases/dbt/models": { title: "Models" },
            "docs/use-cases/dbt/tests": { title: "Tests" },
        }
        const html = (
            await renderVersionedDocBody({
                version: "1.3",
                path: "",
                markdown: `---\ntitle: T\n---\n:::ChildTableOfContents{pageUrl="/docs/use-cases/"}\n:::`,
                children: withGrandchildren,
            })
        ).html
        expect(html).toContain('<ul data-level="1">')
        expect(html).toContain('href="/docs/1.3/use-cases/dbt/models"')
        expect(html).toContain('href="/docs/1.3/use-cases/dbt/tests"')
        // python has no children of its own, so it gets no nested list
        const pythonEntry = html.slice(html.indexOf('href="/docs/1.3/use-cases/python"'))
        expect(pythonEntry.slice(0, pythonEntry.indexOf("</li>"))).not.toContain("<ul")
    })

    it("hides the nested level when max is 0, matching the real component's gate", async () => {
        const withGrandchildren = {
            ...children,
            "docs/use-cases/dbt/models": { title: "Models" },
            "docs/use-cases/dbt/tests": { title: "Tests" },
        ...
        const html = (
            await renderVersionedDocBody({
                version: "1.3",
                path: "",
                markdown: `---\ntitle: T\n---\n:::ChildTableOfContents{pageUrl="/docs/use-cases/" max=0}\n:::`,
                children: withGrandchildren,
            })
        ).html
...
After.`,
    ...
    it("doesn't flag drift for that same component as the page's last, childless node", async () => {
        // Trailing and childless, it's trimmed as residue before componentHtml
        // ever runs and marks it seen — the referencedTags loop must not
        // mistake "trimmed" for "doesn't exist".
        const body = await renderVersionedDocBody({
            version: "1.3",
            path: "",
            markdown: `---
...
describe("componentKey file-name resolution", () => {
    ...
    const componentFileKeys = new Set(
        readdirSync(resolve(dirname(fileURLToPath(import.meta.url)), "../components/content"))
...
describe("versioned-docs component glob", () => {
    ...
    // Each call is matched separately, not merged into one set: the two calls'
    // exclusion lists are meant to be identical (Vite needs the negative
    // pattern spelled out literally in each), so this fails if one drifts from
    // the other instead of only failing when both drift the same way.
    const globCalls = [
        ...page.matchAll(/import\.meta\.glob<\{ default: AstroComponent \}>\(\[([\s\S]*?)\]\)/g),
    ].map((match) => new Set([...match[1].matchAll(/"!(\/src\/components\/[^"]+)"/g)].map((m) => m[1])))

    // Each of these drags Astro's content data layer (50 MB) into the docs
    // worker, which Cloudflare rejects at 64 MB. A new getCollection component
    // under content/ fails here rather than at deploy. Scoped to content/,
    // matching the glob itself: docs/ and common/ aren't globbed at all.
    const coupled = new Set(
        readdirSync(resolve(srcDir, "components/content"))
            .filter((file) => /\.(astro|vue)$/.test(file))
            .filter((file) =>
                readFileSync(resolve(srcDir, "components/content", file), "utf8").includes(
                    "astro:content",
                ),
            )
            .map((file) => `/src/components/content/${file}`),
    )

    it("finds both glob calls", () => {
        expect(globCalls).toHaveLength(2)
    })

    it.each([0, 1])("call %i excludes exactly the components that read the current content collection", (i) => {
        expect(globCalls[i]).toEqual(coupled)
    })
})
