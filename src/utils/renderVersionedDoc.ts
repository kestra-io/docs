...
const HTML_TAGS = new Set([
    ...
    "iframe", "picture", "input", "audio", "link",
])
...
function componentHtml(
    ...
    switch (tag) {
        ...
        case "child-table-of-contents":
            return tableOfContentsHtml(targetDocKey(props, ctx), ctx, props.max) + inner
...
/** A child item's rendered link, `docChildHref`+title, shared by both ToC levels. */
function tocLinkHtml(key: string, meta: DocChildren[string], ctx: RenderCtx): string {
    const title = meta.sidebarTitle ?? meta.title ?? key.split("/").pop() ?? key
    return `<a href="${escapeHtml(docChildHref(ctx.version, key))}">${escapeHtml(title)}</a>`
}

/**
 * Nested link list of a node's children, mirroring ChildTableOfContents.astro:
 * one level of grandchildren under each child, gated the same way the real
 * component gates it — `max` is really a boolean here (0 hides the second
 * level, anything else, including absent, shows it), not a depth limit.
 */
function tableOfContentsHtml(parentKey: string, ctx: RenderCtx, max: unknown): string {
    const items = directDocChildren(ctx.children, parentKey)
    if (!items.length) return ""
    const showGrandchildren = max === undefined || Number(max) > 0
    const links = items
        .map(({ key, meta }) => {
            const grandchildren = showGrandchildren ? directDocChildren(ctx.children, key) : []
            const hasGrandchildren =
                grandchildren.length > 1 ||
                (grandchildren.length === 1 && grandchildren[0].key !== key)
            const nested = hasGrandchildren
                ? `<ul data-level="1">${grandchildren
                      .map((g) => `<li>${tocLinkHtml(g.key, g.meta, ctx)}</li>`)
                      .join("")}</ul>`
                : ""
            return `<li>${tocLinkHtml(key, meta, ctx)}${nested}</li>`
        })
        .join("")
    return `<ul>${links}</ul>`
}
...
export async function renderVersionedDocBody({
    ...
    for (const tag of referencedTags) {
        // Not folded into isKnownComponent: that predicate also drives
        // trimTrailingResidue, and treating these as "known" there would stop
        // it trimming a trailing childless one, same hole as an unresolvable
        // tag. Here it only needs to suppress a false drift warning — a
        // CURRENT_CONTENT_ONLY tag that's the page's last, childless node is
        // trimmed before componentHtml ever runs and marks it seen.
        if (
            !ctx.seenComponents.has(tag) &&
            !isKnownComponent(tag, ctx) &&
            !CURRENT_CONTENT_ONLY.has(tag)
        ) {
            ctx.unknownComponents.add(tag)
        }
    }
