        ...
        for (const alias of plugin.aliases) {
            const match = allElements.find(
                (cls) =>
                    ...
                    cls
                        ?.toLowerCase()
                        .endsWith("." + alias.split(".").pop()?.toLowerCase()),
            )
...
export function redirectAlias(
    ...
    if (pluginType && aliasMapping && !pageList?.includes(slug)) {
        let redirectPath = pageList?.find((page) =>
            page?.endsWith("/" + pluginType),
        )
        ...
        if (!redirectPath) {
            ...
            if (actualClass) {
                redirectPath = pageList?.find((page) =>
                    page
                        ?.toLowerCase()
                        .endsWith("/" + actualClass.toLowerCase()),
                )
            }
        }
