    ...
    <div class="changelog-list">
        <div class="my-6 d-flex align-items-center gap-3">
            <div class="search-input w-100">
                <Magnify />
                <input class="form-control" placeholder="Search releases..." v-model="searchQuery" />
            </div>

            <span class="badge-major filter-btn" :class="{ active: currentFilter === 'major' }"
                @click="setFilter('major')">
                Major
            </span>

            <span class="badge-minor filter-btn" :class="{ active: currentFilter === 'minor' }"
        ...
        <div class="row row-cols-1 row-cols-md-3 g-4" v-if="releasesPaginated.length > 0">
            <div v-for="r in releasesPaginated" :key="r.tag_name" class="col">
                <a class="card h-100 text-decoration-none text-reset release-card"
                    ...
                    <div class="card-body">
                        ...
                        <small class="text-muted mb-2 d-flex align-items-center gap-1">
                            <b><CalendarRange /> Published on</b> {{ new Date(r.published_at).toLocaleDateString() }}
                        </small>
                    </div>
                </a>
            </div>
        ...
        <CommonPaginationContainer v-if="filteredReleases.length > 0" :current-url="currentUrl"
            :total-items="filteredReleases.length" :default-size="12" :show-total="false" @update="handleUpdate" />
    </div>
</template>
