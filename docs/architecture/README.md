...
- The **Internal Storage**: used to store flow data like tasks outputs and flow inputs.
- The **Queue**: used for internal communication between Kestra server components.
- The **Repository**: used to store flows, templates, executions, logs, ... Every internal object is stored by the repository.

Each of these internal components is provided on multiple implementations depending on your needs and your deployment architecture. You may need to install additional plugins to use some implementations.
...
There exist three types of queues:
...
Each server component interact with the Kestra internal components (internal storage, queues and repositories).
...
You can scale Executors as necessary, but as no heavy computations are done in the Executor, this server component does not require a lot of resources (unless you have a very high rate of executions).


### Worker

The **Worker** is where all tasks are run. A Worker will receive a task from the Executor and will process it. As tasks can be nearly anything (heavy computations, simple api calls, etc...), the Worker is essentially a Thread Pool where you can configure the amount of threads you need.

You can scale Workers as necessary, and have many instances on multiple servers, each with its own Thread Pool.

As the worker will execute the tasks, it needs to have access to any external services used by your tasks (database, REST services, message broker, ...).
...
Some triggers may require to have access to external services (triggers based on database query or message subscription for example). If you use such triggers the Scheduler will need to have access to the required external services as it executes the trigger.

### Indexer

The **Indexer** is an optional component only needed when using Kafka and Elasticsearch. It will watch for many Kafka topics (like flows and executions) and will index there content to Elasticsearch [Repositories](#repositories).

### Webserver

The **Webserver** offers 2 main modules in the same component:
- **API**: All the [apis](../api-guide/) that allow to trigger executions for any systems, and to interact with Kestra.
- **UI**: The [User Interface](../user-interface-guide) (UI) is also served by the webserver.

The Webserver interacts mostly with the Repository to deliver a rich API/UI. It also interacts with the Queue in order to trigger new executions, follow executions in real-time, etc...
...
At its heart, Kestra has a plugging system allowing to choose the dependency type that fit your needs.

You can find bellow three example deployment architectures.

### Small-sized deployment

![Kestra Standalone Architecture](./architecture-standalone.svg "Kestra Standalone Architecture")

For small-sized deployment, you can use Kestra standalone server which is a all-in one server component that allows to run all Kestra server components in a single process. This deployment architecture has no scaling capability.

It can be used with a database as the only dependency. This allows to run Kestra with a minimal stack to maintain. We have for now 3 databases available:
- PostgreSQL
- MySQL
- H2
...
For medium-sized deployment where high availability is not a strict requirement, you can use a database (Postgres or MySQL) as the only dependency. This allows to run Kestra with a minimal stack to maintain. We have for now 2 databases available for this kind of architecture as H2 is not a good fit when running distributed components:
- PostgreSQL
- MySQL
...
In this deployment mode, unless all components runs on the same host, you must use a distributed storage implementation like Google Cloud Storage, AWS S3 or Azure Blobs.
...
In order to support higher throughput, and full horizontal and vertical scaling of the Kestra cluster, we can replace the database with Kafka and Elasticsearch. In this case, all the server components can be scaled without any single point of failure.
...
[Kafka](https://kafka.apache.org/) is Kestra's primary dependency in high availability mode. Each of the most important server components in the deployment need to have a Kafka instance up and running. Kafka allows Kestra to be an highly scalable solution.

Kafka configuration options are available [here](../administrator-guide/configuration/kafka/README.md).

##### Kafka Executor

With Kafka, the Executor is a heavy [Kafka Stream](https://kafka.apache.org/documentation/streams/) application. The Executor processes all events coming from Kafka in the right order, keeps an internal state of the execution, and merges task run results coming from the Worker.
It also detects dead Workers and resubmits the tasks ran by a dead Worker.

As the Executor is a Kafka Stream, it can be scaled as needed (within the limits of partitions count on Kafka), but as no heavy computations are done in the Executor, this server component does not require a lot of resources (unless you have a very high rate of executions).
