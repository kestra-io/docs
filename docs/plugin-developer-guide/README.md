...
This template will create a project that will host a group of plugins as we usually create multiple plugins for a technology / a use case.
...
As you can see, there is one generated plugin: the `Example` class that represents the `Example` plugin (a task). 
A project typically host multiple plugins. We call a project a group of plugins, and you can have multiple sub-groups inside a project by splitting plugins on different packages, each package that have a plugin class is a sub-group of plugins.
