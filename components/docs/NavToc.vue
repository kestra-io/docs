...
<script setup lang="ts">
    ...
    const isPluginPage = computed(() => route.path?.startsWith("/plugins") ?? false);
    ...
    const getFixedHeaderOffset = (): number => {
        ...
        for (const sel of selectors) {
            ...
            if (el) {
                ...
                const isFixed = style.position === "fixed" || style.position === "sticky" || Math.round(el.getBoundingClientRect().top) === 0;
    ...
    const activateMenuItem = (item: TocLink, index: number, linkArray: TocLink[]): void => {
        ...
        const prevEl = index > 0 && linkArray[index - 1]?.id
