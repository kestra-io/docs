<template>
    <div class="container-fluid bd-gutter bd-layout">
        <PluginSidebar :plugin-wrapper="rootPlugin" :plugins-without-deprecated="pluginsWithoutDeprecated"
                    ...
                    <div class="title-content d-flex flex-column justify-space-between w-100">
                        ...
                        <MDC v-if="page?.description" :value="page.description">
                            <template #default="mdcProps">
                                ...
                                <ContentRenderer v-else-if="mdcProps?.body" class="desc markdown" :value="mdcProps.body" />
                            </template>
                        </MDC>
                    </div>
                    <div class="title-actions d-flex flex-column gap-2 ps-4">
                        <span v-if="rootPlugin?.license" class="btn enterprise-badge">Enterprise Edition</span>
...
<script setup lang="ts">
    ...
    const currentPageMetadata = computed(() => {
        ...
        if (Array.isArray(meta)) {
            ...
            return meta.find(
                m =>
                    m.group === rootGroup || m.name === rootPlugin.value?.name || m.name === pluginName.value
            ) ?? null;
        }
        return meta;
    });
