...
const [{ color: paletteLight }, { color: paletteDark }] =
    figma.find((p) => p.name === "Colors")?.values || []
...
const checkPalette = (palette1, palette2) => {
    for (const color1 of palette1) {
        ...
        if (!color2 || color2.value !== color1.value) {
            console.error(
                `Color ${color1.name} is different in the two palettes`,
            )
        }
    }
}
...
const getValue = (color, colorIndex) => {
    ...
    if (color.value.length === 9) {
        const alpha =
            Math.round(parseInt(color.value.slice(7), 16) / 2.55) / 100
...
const makePalettes = (palette, paletteName, selector) => {
    ...
    const baseColors = palette
        ...
        .sort((a, b) => {
            const [categoryA, numA] = a.name.split("/")
            const [categoryB, numB] = b.name.split("/")
    ...
    const scss = baseColors
        .map(
            (color) =>
                `$base-${color.name.replace(/\//g, "-")}: ${color.value};`,
        )
        .join("\n")
