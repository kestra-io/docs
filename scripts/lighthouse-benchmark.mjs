...
const METRIC_DEFS = [
  { auditKey: 'largest-contentful-paint', key: 'lcp', label: 'LCP', unit: 's', divisor: 1000, decimals: 2 },
  { auditKey: 'first-contentful-paint', key: 'fcp', label: 'FCP', unit: 's', divisor: 1000, decimals: 2 },
  { auditKey: 'total-blocking-time', key: 'tbt', label: 'TBT', unit: 'ms', divisor: 1, decimals: 0 },
  { auditKey: 'cumulative-layout-shift', key: 'cls', label: 'CLS', unit: '', divisor: 1, decimals: 3 },
  { auditKey: 'speed-index', key: 'si', label: 'Speed Index', unit: 's', divisor: 1000, decimals: 2 },
];
 ...
 * @returns {Promise<import('lighthouse').Result>}
 */
...
function buildMarkdown(output, baseline) {
  const testedAt = new Date(output.timestamp).toISOString().replace('T', ' ').slice(0, 16) + ' UTC';
  ...
  for (const result of output.results) {
    if (result.error) {
      lines.push(`| [${result.label}](${result.path}) | ❌ error | ❌ error | ❌ error | ❌ error |`);
      continue;
    }
...
async function main() {
  ...
  console.log(`Baseline : ${BASELINE_FILE && existsSync(BASELINE_FILE) ? BASELINE_FILE : 'none'}\n`);
  ...
  try {
    for (const page of PAGES) {
      ...
      try {
        ...
        results.push({ path: page.path, label: page.label, scores, metrics });
        console.log(
        ...
        results.push({ path: page.path, label: page.label, scores: { performance: 0, accessibility: 0, 'best-practices': 0, seo: 0 }, metrics: { lcp: 0, fcp: 0, tbt: 0, cls: 0, si: 0 }, error: message });
      }
    }
  } finally {
    await chrome.kill();
  ...
  if (BASELINE_FILE && existsSync(BASELINE_FILE)) {
    try {
      ...
      console.log(`Baseline loaded from ${BASELINE_FILE} (${baseline?.timestamp?.slice(0, 10)})`);
    } catch {
      console.warn(`Warning: Could not parse baseline file ${BASELINE_FILE}, skipping comparison.`);
    }
  }
