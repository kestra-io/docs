...
# Data Storage and Processing in Kestra – Where Data Lives
...
To do so, tasks store data as [output attributes](../05.workflow-components/06.outputs.md) that are then available inside the flow via Pebble expressions like `{{outputs.taskName.attributeName}}`.
...
The same syntax applies to SQL queries, custom scripts, and many more. Check the [Namespace Files](../06.concepts/02.namespace-files.md) documentation for more details.
