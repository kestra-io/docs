...
Check the [Software Requirements](../00.requirements.md) section for the minimum version of each database.
...
The total number of connections opened to the database will depend on your chosen [architecture](../../04.architecture/index.md) used. Each Kestra instance will open a pool of up to the `maximum-pool-size` (10 by default), with a minimum size of the `minimum-idle` (also set to 10 by default).
