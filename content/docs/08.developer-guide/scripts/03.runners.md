...
2. Setting the `runner` property to `PROCESS` will execute your task in a local process on the [worker](../../03.concepts/worker.md) without relying on Docker for container isolation.
...
Head over to the [Secrets](../../03.concepts/secret.md) section to learn more about secrets in Kestra.
