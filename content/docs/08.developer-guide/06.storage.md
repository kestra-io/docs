...
To do so, tasks store data as [output attributes](../07.concepts/outputs.md) that are then available inside the flow via Pebble expressions like `{{outputs.taskName.attributeName}}`.
...
2. The [Blueprints](../01.getting-started/07.ui.md#blueprints) catalog — simply search for the relevant language (e.g. Python, R, Rust)  or use case (*ETL, Git, dbt, etc.*) to find the relevant examples.
...
The [PurgeExecution](/plugins/core/tasks/storages/io.kestra.core.tasks.storages.purgeexecutio) task can purge all the files stored inside the internal context by a flow execution.
