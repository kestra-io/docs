...
Here is an example showing how you can use the `read()` function in a [ClickHouse Trigger](https://kestra.io/plugins/plugin-jdbc-clickhouse/triggers/io.kestra.plugin.jdbc.clickhouse.trigger) to read a SQL query stored as a Namespace File:
...
::alert{type="warning"}
Note: if you use the [Brave browser](https://brave.com/), you may need to disable the Brave Shields to make the Editor work as expected. Specifically, to view the task documentation, you need to set the `Block cookies` option to `Disabled` in the Shields settings: `brave://settings/shields`.
