...
::alert{type="info"}
Note that if your Python script contains any [Pebble expressions](../07.concepts/expression/01.index.md) like e.g. reading [Secrets](../07.concepts/04.secret.md), you need to wrap the `read` function with `render` function like so:
```yaml
