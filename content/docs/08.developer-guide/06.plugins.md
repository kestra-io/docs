...
::alert{type="warning"}
Note that plugin and subplugin **versions** must align with your Kestra instance. You may encounter validation issues during flow creation (e.g. `Invalid bean` response with status 422) when some subplugins are on an older version of Kestra. In that case, you may want to update the file `plugin-yourplugin/gradle.properties` and set the `version` property to the correct Kestra snapshot e.g.:
...
Do not forget to render variables if you need to pass some output from previous variables.
...
- The package in which a plugin is written is called a sub-group of plugins. Sometimes, there is only one sub-group, in which case the group and the sub-group are the same.
