...
To ensure that a block of code won't be parsed by Pebble, you can use the `{% raw %}` and `{% endraw %}` [Pebble tags](../expressions/06.tag/raw.md). For example, the following Pebble expression will return the string `{{ myvar }}` instead of the value of the `myvar` variable:
...
Yes. Kestra uses [Pebble Templates](https://pebbletemplates.io/) along with the execution context to render **dynamic properties**. This means that you can use Pebble expressions (such as [filters](../expressions/03.filter/index.md), [functions](../expressions/04.function/index.md), and [operators](../expressions/05.operator/index.md)) to transform [inputs](../06.workflow-components/06.inputs.md) and [variables](../06.workflow-components/05.variables.md).
