...
| `type` | free-form string used for filtering |
...
An asset is uniquely identified by its `id` and the `tenantId` where you create it. You can attach a namespace to an asset to improve filtering, and to restrict visibility so only users or groups with the appropriate RBAC can access the asset.

## Asset Type

Asset types fall into two categories:

- **Kestra-defined asset types**: When you use the `io.kestra.core.models.assets` model, Kestra automatically captures and manages asset metadata based on the specific plugin. This provides consistent, structured asset information and simplifies tracking across flows and tasks.
...
- **Free-form asset types**: You can define asset types using any string value to represent custom asset categories that fit your organization’s needs. This lets you create and manage your own asset taxonomies, giving you flexibility to describe resources that are not covered by Kestra's standard models.

    > **Note:**  
    > Under the hood, free-form asset types are stored using the model `io.kestra.core.models.assets.External`.
