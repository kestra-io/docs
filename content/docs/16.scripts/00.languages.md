...
You can also pre-install any language using a custom Docker image and use [Process Task Runner](../task-runners/04.types/01.process-task-runner.md) to run that language.
Here is an example with Go:
...
Once you start Kestra containers using `docker compose up -d`, you can create a flow that directly runs Go tasks with your custom dependencies using the `io.kestra.plugin.core.runner.Process` runner:
