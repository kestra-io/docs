...
[Namespaces](../03.concepts/01.flows.md#namespace) are used to group flows and provide structure. Keep in mind, however, that allocation of a flow to a namespace is immutable. Once a flow is created, you cannot change its namespace. If you need to change the namespace of a flow, create a new flow with the desired namespace and delete the old flow. To add another layer of organization, you can use [labels](../03.concepts/01.flows.md#labels), allowing you to group flows by any criteria you want using key-value pairs.
...
description: |
  # Getting Started
  Let's write some **markdown** to `document` our [first flow](https://t.ly/Vemr0) 🚀
...
Learn more about flows in the [Flows](../05.developer-guide/01.flow.md) section.
...
Tasks are defined in form of a **list**. By default, all tasks in the list will be executed **sequentially**, one after the other — the second task will start as soon as the first one finishes successfully.

Kestra provides additional **customization** allowing to run tasks **in parallel**, iterating sequentially or in parallel over a list of items, or to **allow failure** of specific tasks. Those are called [`Flowable` tasks](05.flowable.md) because they define the flow logic.
...
![Autocompletion](/docs/tutorial/fundamentals/autocompletion.gif)
...
1. The **Core tasks** from the `io.kestra.core.tasks.flows` category are commonly used to control the flow logic. You can use them to declare which processes should run **in parallel** or **sequentially**. You can specify **conditional branching**, **iterating** over a list of items, **pausing** or allowing certain tasks to fail without failing the Execution.
2. **Script tasks** are used to run scripts in Docker containers or local processes. You can use them to run Python, Node.js, R, Julia, or any other script. You can also use them to execute a series of commands in Shell or PowerShell. Check the [Script tasks](../05.developer-guide/03.scripts.md) page for more details.
