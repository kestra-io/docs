...
::alert{type="info"}
We assume you followed the [Getting Started](../getting-started.md) guide before this tutorial.
::
...
We identify them by an `id` and a `namespace`. The `id` is a unique identifier inside the [namespace](../concepts/flows#namespace), which is used to group flows. Flows can also have a [description](../developer-guide/flow#document-your-flow) and labels.
...
Discover more about flows in the [Flows](../developer-guide/flow.md) section.
