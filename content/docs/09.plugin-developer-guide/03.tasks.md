...
Here is a simple [Runnable Task](../06.workflow-components/runnable-tasks.md) that reverses a string:
...
String render = runContext.render(format);
```
In order to use [variables](../07.concepts/06.expressions/01.index.md), you need to render the variables, aka: transform the properties with Pebble.
...
You can expose metrics to add observability to your task. Metrics will be recorded with the execution and can be accessed via the UI or as [Prometheus metrics](../10.administrator-guide/03.monitoring#prometheus).
...
[Flowable tasks](../07.concepts/flowable-tasks.md) are the most complex tasks to develop, and will usually be available from the Kestra core. You will rarely need to create a flowable task by yourself.
