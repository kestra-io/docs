...
The [configuration](../10.references/index.md) will be done inside the `KESTRA_CONFIGURATION` environment variable of the Kestra container. You can update the environment variable inside the Docker compose file, or pass it via the Docker command line argument.
...
To resolve this, you need to specify a specific tag for the postgres image. In the example below, we specify `16` as the error above was originally initialized by version 16:
