...
Now you are able to access Kestra at `localhost:8080`. In this production chart, there are no dependencies installed like in previous versions of the Kestra Helm chart. Make sure to configure your instance as needed.
...
To fix this, use `root` to launch the DinD container by setting the following values:
 This fix is now to change to insecure mode   ,,, link to dind in README,,, you have the base, select your mode, change the contexts as you want if you want
```yaml
dind:
  image:
    tag: dind
...
### Disable Docker in Docker and use Kubernetes task runner  Ask about relevance
