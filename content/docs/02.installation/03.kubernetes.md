...
3. `kestra-operator`: this is a separate chart for installing the operator (Enterprise Edition).
...
:::alert{type="info"}
**Enterprise Edition images** — authenticate with the Kestra registry before pulling images or creating Kubernetes secrets:

```bash
docker login registry.kestra.io --username $LICENSEID --password $FINGERPRINT
```

Use `registry.kestra.io/docker/kestra-ee:latest` for the newest image, or pin a specific version such as `registry.kestra.io/docker/kestra-ee:v1.0`. Review the [Enterprise documentation](../06.enterprise/index.md) and [configuration requirements](../06.enterprise/05.instance/index.md) before deploying to production.
:::

:::alert{type="info"}
To manage flows declaratively via Kubernetes CRDs, install the [Kestra Kubernetes Operator](../version-control-cicd/cicd/07.kubernetes-operator.md) (Enterprise Edition) alongside your deployment.
:::
...
You'll see a pod has been started:
...
To access the UI, first get the pod name:

```bash
export POD_NAME=$(kubectl get pods --namespace default -l "app.kubernetes.io/name=kestra,app.kubernetes.io/instance=my-kestra,app.kubernetes.io/component=standalone" -o jsonpath="{.items[0].metadata.name}")
```
Then port-forward from localhost:

```bash
kubectl port-forward $POD_NAME 8080:8080
```

Open `http://localhost:8080` in your browser to reach the UI.
...
The above configuration enables separate pods for each Kestra component (webserver, executor, indexer, etc.) and disables the combined standalone pod. This offers more scalable, granular resource allocation of each component depending on workflow demand and how intensely each component is utilized.

To confirm these changes and re-deploy, save the `values.yaml` and upgrade your Helm release:
...
Both must be valid YAML that is merged as the [Kestra configuration](../configuration/index.md) file. The following is an example `configurations` setup (there must be values in order to run Kestra with the `kestra` chart):
