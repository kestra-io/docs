...
2. `kestra-starter`: this chart comes with dependencies such as PostgreSQL, Docker DinD, and MinIO that are maintained by the user -- not recommended for production installations.
...
The `kestra-starter` chart can also deploy the following related services:
...
Next, set the Kubernetes task runner as the default method for running [script tasks](../16.scripts/index.md):
