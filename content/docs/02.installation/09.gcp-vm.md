...
## Overview
This guide provides instructions for deploying Kestra on Google Cloud Platform (GCP). We use Compute Engine with Docker to host Kestra server, a PostgreSQL Cloud SQL database, and Cloud Storage as storage backend.
...
If you are just testing or would like to be able to delete your instance and all of it's data, then expand the `Data Protection` on the left hand side and make sure `Enable deletion protection` is UNCHECKED.
