...
::alert{type="warning"}
If after having executed the `podman machine init` you get a `Error: exec: "qemu-img": executable file not found in $PATH`, you should install qemu before:
- with a `brew install qemu` on MacOS
- a `sudo apt install qemu-utils` on Debian / Ubuntu
- a `sudo dnf install qemu-img` on Fedora / CentOS / RHEL
- a `sudo pacman -S qemu` on Arch Linux
::
