...
- Specify a configuration file to read through `--config` (or `-c`) option. 
- Put your whole configuration in the environment variable `KESTRA_CONFIGURATION`

To start Kestra with a dedicated configuration file, use the command below:  

```shell
./kestra server local --config confs/application.yml
```

By default, Kestra reads configuration files from `${HOME}/.kestra/config.yml`.

If you are using `KESTRA_CONFIGURATION` environment variable, you need to have a directory called `confs` in the directory where you run Kestra, as your configuration will be stored in this location.

Configuration options are available in the [Configuration Reference](../configuration/index.md). You can also see the default configuration available on [GitHub](https://github.com/kestra-io/kestra/blob/develop/cli/src/main/resources/application.yml).
