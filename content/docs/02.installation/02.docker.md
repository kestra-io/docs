...
Once you have Docker running, you can start Kestra in a single command (*if you run it on Windows, make sure to use [WSL](https://docs.docker.com/desktop/wsl/)*):
...
After creating the configuration file, change the command to mount the file to the container and start Kestra; we also adjust the Kestra command to start a standalone version as we now have a PostgreSQL database as a backend.
...
You can adjust the Kestra configuration with `KESTRA_CONFIGURATION` passed to the Docker container via the `-e` options.
...
The `kestra/kestra:*` images contain all Kestra [plugins](/plugins) in their **latest version**. The `kestra/kestra:*-no-plugins` images do not contain any plugins. We recommend using the `kestra/kestra:*` version to take advantage of all the available plugins in your flows.
...
- `kestra/kestra:develop-no-plugins` if you prefer to use only your custom plugins, it is recommended to download all plugins into a local directory and then cherry-pick only those you want to keep.

::alert{type="info"}
For more information on how to install plugins, refer to the [Install plugins from a Docker image](../installation/standalone-server.md#install-plugins-from-a-docker-image) section.
::
