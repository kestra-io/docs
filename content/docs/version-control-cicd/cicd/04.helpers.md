...
:::alert{type="info"}
If helpers feed into flows deployed via CI/CD, add the [`system.readOnly`](../../05.concepts/06.system-labels.md#systemreadonly) label set to `"true"` so those flows remain immutable in the UI. This is especially recommended for critical production flows:
