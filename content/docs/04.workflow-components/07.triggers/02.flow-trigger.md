...
::alert{type="info"}
It is best practice when using a flow trigger to use `preconditions.flows.states` when defining states for one specific flow rather than the `states` property.
::
