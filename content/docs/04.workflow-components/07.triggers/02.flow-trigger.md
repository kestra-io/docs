...
::alert{type="info"}
It is [best practice](../../14.best-practices/0.flows.md#flow-trigger-on-state-change) when using a flow trigger to use `preconditions.flows.states` when defining states for one specific flow rather than the `states` property.
::
