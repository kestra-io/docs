...
Any errors in the `afterExecution` block will not change the state of the flow from Success to Failed, and will not trigger a flow that relies on ExecutionStatus = Failed.
  ...
  - id: t2
    ...
    errors:
      - id: sendAlert
        ...
        payload: |
          {
            "text": "Flow {{ flow.namespace }}.{{ flow.id }} with execution ID {{ execution.id }} failed"
          }
```
