...
Any errors in the afterExecution block will not change the state of the flow from Success to Failed, and will not trigger the flow that relies on ExecutionStatus = Failed.
...
afterExecution:
  - id: t2
    ...
    errors:
      - id: sendAlert
        type: io.kestra.plugin.notifications.slack.SlackIncomingWebhook
        url: "{{ secret('SlackChannel') }}"
        payload: |
          {
            "text": "Flow {{ flow.namespace }}.{{ flow.id }} with execution ID {{ execution.id }} was failed"
          }
```
