...
Flowable tasks use [expressions](../../expressions/index.md) from the execution context determine which tasks run next. For example, you can use the outputs of a previous task in a `Switch` task to decide which task to run next.
