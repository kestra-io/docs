...
::alert{type="warning"}
Don't use Outputs to fetch any sensitive data (passwords, secrets, API tokens, ...). Fetching Secrets from an external Secrets Manager via a task imposes a significanr security risk. All data fetched via outputs is **stored in clear text in multiple places** (incl. the backend database, internal storage, logs, API requests). For secure secrets management, use **exclusively** [Secrets](../05.concepts/04.secret.md) provided in [Kestra EE](../06.enterprise/secrets.md) or [Kestra Cloud](/cloud/).
::
