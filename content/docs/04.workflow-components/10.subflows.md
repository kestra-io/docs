...
::alert{type="warning"}
Recursive flows are not supported. Kestra doesn’t allow a flow to call itself (directly or indirectly). Any cycle **(A→A or A→B→A)** makes the flow invalid.  
**Do instead:** Use **[ForEach](/plugins/core/flow/io.kestra.plugin.core.flow.foreach**)** and **branching** tasks to iterate or split work without creating cycles.
::
