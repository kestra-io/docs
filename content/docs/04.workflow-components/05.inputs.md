...
Variables are more like constant values to be used within the flow. They have the same behaviour as an input during execution but they can't be overridden once the execution starts. Variables must be defined before execution whereas inputs can be set when the execution starts. You can set defaults for inputs, so it's optional to provide values when the execution starts.

Variables are best suited for values that you don't want to change, and are used in multiple places within the flow.
