...
As of version 0.24, with the new HTTP function, you can make the input dropdowns of `SELECT` and `MULTISELECT` types dynamic by fetching options from an external API directly. This proves valuable when your data used in dropdowns changes very frequently, or when you already have an API serving that data for existing applications.
