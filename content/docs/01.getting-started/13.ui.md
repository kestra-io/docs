...
If you launch Kestra from our Docker Compose file, as explained in the [Getting Started](/index.md) guide, it will be available on http://localhost:8080.
...
The Gantt tab allows to see each task's durations. From this interface, you can replay a specific task, see task source code, change task status, or look at task [metrics](../07.concepts/execution.md#metrics) and [outputs](../07.concepts/execution.md#outputs).
...
The "Eval Expression" box allows to evaluate [expressions](../07.concepts/06.expressions/01.index.md) on those task outputs. It's a great way to debug your flows.
...
For example, you can use the "Eval Expression" feature to deep-dive into your tasks' outputs and play directly with [variable operations](../07.concepts/06.expressions/02.basic-usage.md).
...
- The **Variables** tab allows setting [variables](../07.concepts/06.expressions/01.index.md#namespace-variables-ee) scoped to a namespace.
- The **Task defaults** tab allows setting task default scoped to a namespace.
- The **Secrets** tab allows setting secrets accessible via the [secret function](../07.concepts/06.expressions/04.function/secret.md).
