...
This tutorial will guide you through **key concepts** in Kestra. We'll build upon the "Hello world" flow from the [Getting Started](/index.md) section, and we'll gradually introduce new concepts including `namespaces`, `tasks`, parametrization with `inputs` and scheduling using `triggers`.
