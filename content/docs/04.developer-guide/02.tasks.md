...
Task properties can be static or dynamic. Dynamic task properties can be set using [variables](./variables/index.md). To know if a task property is dynamic, read the task's documentation.
...
Flowable Tasks use context [variables](./variables/index.md) to define the next tasks to run. For example, you can use the [outputs](./outputs.md) of a previous task in a `Switch` task to decide which task to run next.
...
::alert{type="info"}
You can access the output of a sibling task with `{{outputs.sibling.value}}`, see [Lookup in sibling tasks](../developer-guide/outputs#lookup-in-sibling-tasks)
::
...
::alert{type="info"}
You can access the output of a sibling task with `{{outputs.sibling[taskrun.value].value}}`, see [Lookup in sibling tasks](../developer-guide/outputs#lookup-in-sibling-tasks)
::
...
You can pass flow [outputs](./outputs.md) as [inputs](./inputs.md) for the triggered flow (those must be declared in the sub-flow).
