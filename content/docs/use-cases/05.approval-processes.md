...
To ensure the entire execution stops when a user kills a paused task manually from the UI or API, you can add an explicit `Kill` task after the `Pause` task:
