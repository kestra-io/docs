...
You can use expressions inside your flow definition by using a special syntax: `{{ expression_name }}`.
...
Flow inputs will be available as expressions via `{{ inputs.myinput }}`, and task's output attributes will be available as expressions via `{{ outputs.task_id.output_attribute }}`.

Most expressions will be stored inside the context of the execution and directly passed from task to task. Some expressions are of a special type and will be stored inside Kestra's internal storage. Those expressions will be automatically fetched from the internal storage when needed.

You can use expressions to set any task property. They use [Pebble Templating](https://pebbletemplates.io/) with Kestra's special context system, allowing powerful task composition inside a flow.
...
|  `{{ parents }}` | The list of parent tasks, only available with tasks inside a ([Flowable Task](../02.tasks.md#flowable-tasks)). See [Parents expressions](./02b.using-expressions.md#parents-with-flowable-task) for its usage.                              |

If a [schedule](../08.triggers/01.schedule.md) event triggers the flow, these expressions are also available:
...
If a [flow](../08.triggers/02.flow.md) event triggers the flow, these expressions are also available:
