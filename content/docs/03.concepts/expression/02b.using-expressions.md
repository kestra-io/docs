...
When using nested [Flowable Tasks](../flowable-tasks.md), only the direct parent task is accessible via `taskrun.value`. To access a parent task higher up the tree, you can use the `parent` and the `parents` expressions.
