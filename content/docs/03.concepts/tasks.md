...
|`workerGroup.key`|To execute this task on a specific [Worker Group (EE)](../03.concepts/worker-group.md).|
...
Task properties can be static or dynamic. Dynamic task properties can be set using [variables](./expression/01.index.md). To know if a task property is dynamic, read the task's documentation.
