...
## Overview
[Task Runners](./index.md) and [Worker Groups](../../06.enterprise/worker-group.md) both **offload compute-intensive tasks to dedicated workers**. However, **worker groups have a broader scope**, applying to **all tasks** in Kestra, whereas **task runners** are limited to **scripting tasks** (Python, R, JavaScript, Shell, dbt, etc. — see the [full list here](01.overview.md#plugins-supporting-task-runners)). Worker groups can be used with any plugins.
...
![default_worker_group](../../concepts/default_worker_group.png)
