...
Follow the instructions in the [aws-batch README](...) in the [terraform-deployments](...) repository to provision resources using Terraform. You can also use [this blueprint](/blueprints/aws-batch-terraform-git), which creates all required resources in a single Kestra workflow execution.
