...
Apart from **core tasks**, the [plugins library](https://kestra.io/plugins) provides a wide range of integrations. Kestra has built-in plugins for data ingestion, data transformation, interacting with databases, object stores, or message queues, and the list keeps growing with every new release. On top of that, you can also [create your own plugins](/docs/developer-guide/plugins) to integrate with any system or programming language.
