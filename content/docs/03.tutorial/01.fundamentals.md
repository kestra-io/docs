...
Kestra provides additional **customization** allowing to run tasks **in parallel**, iterating (_sequentially or in parallel_) over a list of items, or to **allow failure** of specific tasks. These kind of actions are called [`Flowable`](./05.flowable.md) tasks because they define the flow logic.
