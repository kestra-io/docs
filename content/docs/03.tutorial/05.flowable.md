...
In addition to running independent processes in parallel, you may want specific tasks to run only if certain conditions are met; otherwise, another action occurs. Core tasks from the `io.kestra.plugin.core.flow` category control flow logic. Use them to run tasks in parallel or sequentially, branch conditionally, iterate over items, pause, or allow specific tasks to fail without stopping the execution.
