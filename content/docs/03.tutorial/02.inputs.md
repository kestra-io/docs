...
From the example above, you'll notice the ``{{ inputs.user }}`` syntax in the task message. Inputs can be accessed in any task using the following [expression syntax](../expressions/index.md) `{{ inputs.input_name }}`. Replace `input_name` with the `id` of your input, and the value is accessable to any downstream tasks.
