...
Check the [error handling](../08.developer-guide/07.errors-handling.md) page for more details.
...
For a centralized namespace-level alerting, we recommend adding a dedicated monitoring workflow with one of the above mentioned notification tasks and a [Flow trigger](../07.concepts/05.triggers/flow-trigger.md). Below is an example workflow that automatically sends a Slack alert as soon as any flow in the namespace `prod` fails or finishes with warnings.
...
The [Retries](../07.concepts/retries.md) page provides more details on possible retry configurations.
