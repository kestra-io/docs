...
|`logLevel`| To define the minimum flow log level for which a flow log will be inserted into the database. By default all logs are kept. |
...
This will execute the subflow `dev.subflow` for each batch of 10 items. 
The subflow must define an input named `file` that will be set to the URI of the internal storage file containing the batch of items.
...
This task can also fetch files from [namespace files][namespace files](./namespace-files.md) and make them available for all sub-tasks.
