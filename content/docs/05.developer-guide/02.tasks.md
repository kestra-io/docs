...
This task allows defining dependencies between tasks by creating a directed acyclic graph (DAG) without bothering with the graph structure. List your tasks and their dependencies, and Kestra will figure out the rest.
...
The Worker task is deprecated in favor of the WorkingDirectory task; see below.

### WorkingDirectory

By default, Kestra will launch each task on a new working directory and possibly on different workers if multiple ones exist.

This task will run all sub-tasks sequentially; in the same working directory, allowing the reuse of previous tasks files on the next tasks. For this, all sub-tasks will be launched on the same worker.

This task can be useful when working with large filesystem operations.
