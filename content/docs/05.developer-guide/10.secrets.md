...
You can leverage the `secret()` function, described in the [function section](./03.variables/04.function/secret.md), to retrieve the sensitive variable within your flow code.
...
If you set the environment variable in a Docker Compose file, it would be:
...
This variable can then be used in a flow using:
...
You should see the output `password` in your logs when executing that flow. This is because at runtime, that secret will be decoded. 
