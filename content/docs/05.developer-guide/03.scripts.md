...
Each of those languages have a dedicated tag in the [Blueprints](../00.getting-started/07.ui.md#blueprints) catalog. Check available blueprints to get started with those tasks. The section below explains the recommended usage patterns and common examples.
...
Then, use that image in your `docker-compose.yml` file, as shown in the [Getting started](../00.getting-started/index.md) guide.
...
If you want to generate files in your script to make them available for download and use in downstream tasks, you can leverage the `outputDir` [expression](../03.concepts/06.expressions/01.index.md). Files stored in that directory will be persisted in Kestra's internal storage. Here is an example:
