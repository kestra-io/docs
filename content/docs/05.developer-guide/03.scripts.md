...
|Language|Default image|beforeCommands example|Script example|Commands example|Outputs|Metrics|Task type (full class name)|
|---|---|---|---|---|---|---|---|
|Python|python|pip install requests kestra|print("Hello World!")|python [hello.py](http://hello.py)|Kestra.outputs({'data': data})|Kestra.counter('nr_rows', len(df))|io.kestra.plugin.scripts.python.Commands/Script|
|R|r-base|Rscript -e "install.packages('dplyr')"|print("Hello World!")|Rscript hello.R|TBD|TBD|io.kestra.plugin.scripts.r.Commands/Script|
|Node.js|node|npm install json2csv|console.log('Hello World!');|node hello.js|Kestra.outputs({'status': d.status, 'text': d.data})})|TBD|io.kestra.plugin.scripts.node.Commands/Script|
|Shell|ubuntu|apt-get install curl|echo "Hello World!"|./hello.bash|echo '::{"outputs":{"nr_rows":800}}::’|echo '::{"metrics":[{"name":"nr_rows","type":"counter","value":800,"tags":{"schema":"analytics"}}]}::’|io.kestra.plugin.scripts.shell.Commands/Script|
|Powershell|[http://mcr.microsoft.com/powershell](http://mcr.microsoft.com/powershell)|Install-Module -Name ImportExcel|Write-Output "Hello World!"|.\hello.ps1|TBD|TBD|io.kestra.plugin.scripts.powershell.Commands/Script|
...
In contrast to the `Commands` tasks, the `Script` task doesn't require using Git to store your script code. Instead, you just define it inline in your YAML workflow definition along with any other configuration. 
...
4. You can also *optionally* use the `beforeCommands` property to install libraries used in your inline script. Here, we use the command `pip install requests kestra` to install two `pip` packages required by this script, but not being available by default in the base image `python:3.11-slim`. 
5. Also here, the `LocalFiles` task can be used to output a file generated as a result of a script. This script outputs a JSON file and a downstream task loads that file as a document to a MongoDB collection. 
...
- **Simplicity**: you don't need to commit files to external repositories. They are stored and **versioned** using Kestra's revision history along with your orchestration code.
...
- Adding programming language code into a configuration file such as YAML makes that code more difficult to read, test and review. It lacks the syntax highlighting and testability you would otherwise get from your IDE when writing e.g. Python script in a `.py` file rather than in a `.yml` config.
          ...
          config: |
            {
              "auths": {
                  "https://index.docker.io/v1/": {
                      "username": "annageller",
                      "password": "dckr_pat_WfOmcNCeDshPOyKndP-fR_3zSHc"
# note that password can also be a generated token
                  }
              }
            }
```
...
This is useful if you run Kestra locally without Docker and want to access your local files and local environments e.g. locally configured Conda virtual environments.
...
TBD Explain pydata, rdata and their purpose
...
You can also use other languages as long as their dependencies can be packaged into a Docker image and their execution can be triggered from a command line.
