...
1. Even though it's not required, it's best to start with a `flows.WorkingDirectory` task. This task allows you to configure additional **input** or **output** files using the `LocalFiles` task. The example below uses a SQL file as input.
...
Overall, we recommend using `git.Clone` and `Commands` tasks for advanced production workloads. However, the `Script` tasks offer a great alternative for simple use cases.
...
To use the `git.Clone` task in your flow, make sure to add it as the first child task of the `WorkingDirectory` task. Otherwise, you’ll get an error: `Destination path "xyz" already exists and is not an empty directory`. This happens because you can only clone a GitHub repository into an empty working directory. 
...
All tasks within the `WorkingDirectory` can use output files from previous tasks without having to use the `{{outputs.taskId.outputAttribute}}` syntax.
...
1. `counter` expressed in **Integer** or **Double** data type; it measures a countable number of rows/bytes/objects processed in a given task
2. `timer` expressed in **Double** data type; it measures time to process specific computation in seconds
...
- the **accuracy score** of a trained ML model in order to compare this result across multiple workflow executions (*e.g. you can see average or max value across multiple executions*),
