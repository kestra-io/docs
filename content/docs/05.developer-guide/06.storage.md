...
Kestra's primary purpose is to orchestrate data processing via [tasks](03.concepts/tasks.md), so data is central to each [flow's](01.flow.md) execution.
...
To do so, tasks store data as output attributes that are then available inside the flow via Pebble expressions like `{{outputs.taskName.attributeName}}`. More information about outputs can be found [here](./outputs.md).
