...
By default all scripting tasks run in isolated containers using the `DOCKER` process. Setting the `runner` property to `PROCESS` will execute your task in a local process on the Worker without relying on Docker for process isolation.
...
Explain pydata, rdata and their purpose
