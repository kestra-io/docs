...
Here, there's no overlap between the two conditions. The first condition will only match executions in the `company.analytics` namespace, while the second condition will only match executions from the `cleanup` flow in the `company.monitoring` namespace. If you want to match executions from the `cleanup` flow in the `company.monitoring` namespace **or** any execution in the `product` namespace, make sure to add the `OrCondition`.


## Monitoring

By default, Kestra exposes a monitoring endpoint on port 8081. You can change this port using the `endpoints.all.port` property in the [configuration options](/docs/configuration-guide).
...
Kestra metrics use the prefix `kestra`. This prefix can be changed using the `kestra.metrics.prefix` property in the [configuration options](/docs/configuration-guide/metrics).
...
Kestra exposes internal endpoints on the management port (8081 by default) to provide status corresponding to the [server type](/docs/architecture/server-components):
