...
Server components can run independently from each other. Each of them are communicating through the database.

Here is a sampled Docker Compose configuration file running Kestra services with replicas on the Postgre database backend. 
...
1. Runing Kestra services on dedicated machines. For examples, running the webserver, the scheduler and the executor on one VM and running one or more workers other instances.
2. Using Kubernetes and Helm charts. Please find more instruction about these in [this part of the documentation](../02.installation/03.kubernetes.md).
