---
title: Deployment on your AWS, GCP or Azure with Restack
---

[Restack](https://restack.io) provides the tools and infrastructure to build and deploy open-source products in your cloud without DevOps.
...
To deploy Kestra with Restack:
  ...
  - [Connect your AWS](#connect-your-aws-account).
  - [Connect your GCP](#connect-your-gcp-account).
  - [Connect your Azure](#connect-your-azure-account).
...
1. Log into your AWS Management Console as a user with administrator privileges and go to the IAM console.
...
3. In Select type of trusted entity, click the Custom trust policy

![](https://cdn.sanity.io/images/ev3amoz3/production/666cb5015b3c5a29e6eef6216bdd97ef67297dc8-1770x1110.png)

4. Copy and Paste into the custom policy field the following. You also can copy it directly from the Restack user interface modal.
...
5. Click the Next button and Add AdministratorAccess Permissions.

![](https://cdn.sanity.io/images/ev3amoz3/production/2685ee99b153f46b38f99607407dfba72c5c6522-2000x1160.png)

6. Click the Next button and Add Role Name `restack`, Review, and Create button.

![](https://cdn.sanity.io/images/ev3amoz3/production/fb14345d290077195b986cb3931c0d39689dca6f-2000x1255.png)

7. From IAM > Role overview, navigate to the role you just created and copy the ARN from there.

![](https://cdn.sanity.io/images/ev3amoz3/production/e3750c4fb52873a781e328e81c70237d1968d5da-2560x1440.png)

8. Copy the ARN to Restack's user interface and check by clicking on 'Connect cloud' if Restack can successfully assume the role in your AWS account.
...
`sts:AssumeRole` is an AWS Security Token Service (STS) API operation that allows an AWS Identity and Access Management (IAM) user or role in one account to assume a role in another AWS account. This allows the user or role to access resources in the second account that they would not normally have access to.
...
Connect your GCP account in a few steps to Restack.
...
1. Create a new service account in your Google project, you created to connect with Restack

![](https://cdn.sanity.io/images/ev3amoz3/production/43f688c517cfca2c2e430d17d0069ed3fb74b2e8-6016x3384.png)

2. Copy connect@restackio.iam.gserviceaccount.com

3. Navigate to https://console.cloud.google.com/ and to your project-id

4. Go to IAM & Admin > Service Accounts

Create a new service account in your project
Service account name: `restack`
Service account ID: `restack`

5. Grant this service account access to the project with Owner role

![](https://cdn.sanity.io/images/ev3amoz3/production/e30384b7512b54dd7770562d3ab5c10c05eecf6c-6016x3384.png)

6. Grant connect@restackio.iam.gserviceaccount.com access as user to this service account with the following permissions:

- `Service Account Token Creator`

![](https://cdn.sanity.io/images/ev3amoz3/production/4d051eea7ecfd580fd1d5d3f730cebb238e17f3f-6016x3384.png)

7. Click on ‘Done’ to finish the creation of the service account

8. Add your project id and the email of your service account to Restack's user interface to add your cloud and test to connect successfully.

![](https://cdn.sanity.io/images/ev3amoz3/production/bfc406eb19cd1c67245fbd8d7c8e84ceafc7e020-1176x1920.png)

## Connect your Azure account

Connect your Azure account in a few steps to Restack.

> Requirements
- Active Azure account
- Active Azure subscription
- Active Azure user with "Owner" role

1. Create a new App Registration in your Azure Active Directory to connect with Restack.
...
2. Copy the Application (client) ID and paste in Restack UI as the Client ID.

![](https://cdn.sanity.io/images/ev3amoz3/production/392b15df09de170e1bb7632880f42ff093852853-2564x1730.png)

3. Copy the Directory (tenant) ID and paste in Restack UI as the Tenant ID.

![](https://cdn.sanity.io/images/ev3amoz3/production/cb0e76db65825a3d097b764735e6de29d68195ff-2590x1728.png?w=450)

4. For you newly created App Registration add an API Permission called "user_impersonation".

![](https://cdn.sanity.io/images/ev3amoz3/production/d3c38be874bce1bfe63c60a1bd6a2da75b77cce5-3024x1648.png)

5. For your App Registration go to "Certificates & secrets" and create a new client secret. Copy the value of the secret and paste in Restack UI as the Client Secret.

![](https://cdn.sanity.io/images/ev3amoz3/production/2cc097fd4e153bab779de038f153a83bb2a38c59-3024x1648.png)

6. Go to your "Subscriptions", choose the subscription you want to use for Restack. In "Access Control (IAM)" section access "Role assignments" and add a new role assignment. Choose the "Owner" role (Privileged administrator roles) & Azure Kubernetes Service RBAC Cluster Admin (Job function roles) and click "Next".
