...
### Un-processable execution

Sometimes, things can go wrong in an unattended manner; for such extreme issues, we added a way to skip an execution that Kestra is not able to process.
...
Those can be seen on the **Administration -> Triggers** page. On this page, you can unlock a trigger if it is locked. Be careful that if it's not really locked, there is a risk or concurrent trigger evaluation or flow execution for this trigger.
