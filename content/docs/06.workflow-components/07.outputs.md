  ...
  plugins:
    configurations:
      - type: io.kestra.core.tasks.flows.Subflow
        values:
          flowOutputs:
            enabled: false # for backward-compatibility -- true by default
...
Once the `flowOutputs` configuration is set to `enabled: false`, you can use the old behavior of defining outputs within the parent flow.
