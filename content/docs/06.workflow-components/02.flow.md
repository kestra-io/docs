...
A flow can also have [`inputs`](inputs.md), [error handlers](errors.md), and [`triggers`](05.triggers/index.md).
...
[Runnable Tasks](../08.developer-guide/02.tasks.md#runnable-tasks) handle computational work in the flow. For example, file system operations, API calls, database queries, etc. These tasks can be compute-intensive and are handled by [Workers](../07.concepts/worker.md).
...
[Flowable Tasks](../07.concepts/flowable-tasks.md) only handle flow logic (branching, grouping, parallel processing, etc.) and start new tasks. For example, the [Switch task](/plugins/core/tasks/flows/io.kestra.core.tasks.flows.switch) decides the next task to run based on some inputs.

A Flowable Task is handled by [Executors](../07.concepts/executor.md) and can be called very often. Because of that, these tasks cannot include intensive computations, unlike Runnable Tasks. Most of the common Flowable Tasks are available in the default Kestra installation.
...
Inputs are parameters sent to a flow at execution time. It's important to note that inputs in Kestra are [strongly typed](../07.concepts/inputs.md#input-types).
...
Inputs of type `FILE` will be uploaded to Kestra's [internal storage](../07.concepts/internal-storage.md) and made available for all tasks.

Flow inputs can be seen in the **Overview** tab of the **Execution** page.

## Outputs

Each task can produce an output that may contain multiple properties. This output is described in the plugin documentation task and can then be accessible by all the following tasks via [variables](06.expressions/02.basic-usage.md).

Some outputs are of a special type and will be stored inside Kestra's [internal storage](../07.concepts/internal-storage.md). Successive tasks may still use them as Kestra will automatically make them available for all tasks.

Tasks outputs can be seen in the **Outputs** tab of the **Execution** page. If an output is a file from the [internal storage](../07.concepts/internal-storage.md), it will be available to download.
...
[Listeners](../07.concepts/listeners.md) are special tasks that can listen to the current flow, and launch tasks *outside the flow*, meaning launch tasks that are not part of the flow.
...
[Templates](../08.developer-guide/03.concepts/templates.md) are lists of tasks that can be shared between flows. You can define a template and call it from other flows. Templates allow you to share a list of tasks and keep them updated without changing all flows that use them.
