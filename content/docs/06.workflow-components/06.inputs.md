...
- `SECRET`: a `SECRET` input is a string that is encrypted and stored in the database. It is decrypted at runtime and can be used in all tasks. The value of a `SECRET` input is masked in the UI and in the execution context. Note that you need to set the [encryption key](../11.configuration-guide/encryption.md) in your Kestra configuration before using it.
