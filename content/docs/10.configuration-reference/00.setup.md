...
To choose another storage type, you will need to configure the `kestra.storage.type` option. Make sure to download the corresponding storage plugins first. The following example configures [Google Cloud Storage](../10.configuration-guide/02.storage.md#gcs) for internal storage.
...
Details about storage configuration can be found [here](../10.configuration-guide/02.storage.md).
...
For more details, check the [database configuration](../10.configuration-guide/01.database.md) and the [Elasticsearch configuration](../10.configuration-guide/elasticsearch.md).

## Micronaut configuration

Given that Kestra is a Java-based application built on top of Micronaut, there are multiple [Micronaut settings](../10.configuration-guide/micronaut.md) that you can configure based on your needs.
