...
::alert{type="info"}
For a full list of properties available in the `AzureBatchTaskRunner`, check the [Azure plugin documentation](/plugins/plugin-azure/task-runners/runner/io.kestra.plugin.azure.runner.AzureBatchTaskRunner) or explore the same in the built-in Code Editor in the Kestra UI.
::