...
How to use `aws.runner.Batch` to run your tasks on AWS ECS Fargate.
...
Last thing we'll need is a storage bucket. To do this: 

1. Select S3 from Services then select **Create bucket**. 
2. Next you'll need to add a name and leave everything else as a default value.
3. Scroll to the bottom and select **Create bucket**.
