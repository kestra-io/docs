...
::alert{type="info"}
For a full list of properties available in the `AwsBatchTaskRunner`, check the [AWS plugin documentation](/plugins/plugin-aws/task-runners/runner/io.kestra.plugin.aws.runner.AwsBatchTaskRunner) or explore the same in the built-in Code Editor in the Kestra UI.
::
