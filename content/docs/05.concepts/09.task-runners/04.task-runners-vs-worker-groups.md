...
## Overview
Task runners and worker groups can both be used to **offload compute-intensive tasks to dedicated workers**. However, worker groups have a broader scope and can be applied to all tasks in Kestra, while task runners can only be used in scripting tasks (Python, R, JavaScript, Shell, dbt, etc.). You can use worker groups with any plugins.

For example, if you want to query an on-premise SQL Server database running on a different server than the Kestra server, you can use a worker group to target a worker that has access to that server in your SQL Server Query task. At the same time, worker groups can fulfill the same use case as task runners by distributing the load of scripting tasks to dedicated wonprkers that have enough resources and the right dependencies (_incl. hardware, region, network, operating system_) to run them.
...
Finally, worker groups feature requires a [commercial license](https://kestra.io/pricing) while task runners are available in the open-source version of Kestra.
...
- Execute tasks and polling triggers on specific servers (e.g., a VM with access to your on-prem DB or a server with preconfigured CUDA drivers).
...
And here are common use cases in which Task Runners can be beneficial:
...
```shell
kestra server worker --worker-group=workerGroupKey --server=your_ee_host --api-token=your_ee_api_token
```
