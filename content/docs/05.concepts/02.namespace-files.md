...
For the second task, the `test.py` file in the `dev.test` namespace will be used. When using the `read()`, the priority is changed from the ordering to whatever the current namespace in the task is running, in this case `dev.test`.

Lastly, you can specify in the `read()` function which namespace to use such as `"{{ read('test.py', namespace='company') }}"`. This ensures priority is given to whatever you specify without having to consider ordering or current namespace.
