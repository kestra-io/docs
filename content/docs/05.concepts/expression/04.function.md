...
Reading internal storage files is restricted to the **current execution**. Specifically, those are files created by the current flow's execution or the parent flow execution (for flows triggered by a [Subflow](/plugins/core/tasks/flows/io.kestra.core.tasks.flows.subflow) task or a [ForEachItem](/plugins/core/tasks/flows/io.kestra.core.tasks.flows.foreachitem) task).
