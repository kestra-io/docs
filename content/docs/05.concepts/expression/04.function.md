...
The `decrypt` function is used to decrypt the values. AES-256 encryption key is supported for this function.

```twig
{{ decrypt(secret_key, encrypted_value) }}
```

## encrypt

The `encrypt` function is used to encrypt the values. AES-256 encryption key is supported for this function.

```twig
{{ encrypt(secret_key, 'Hello World!') }}
```

## from Ion

The `fromIon` function will convert any Ion stringto an object allowing to access its properties using the dot notation:

```twig
{{ fromIon('[1, 2, 3]')[0] }}
{# results in: '1' #}

{{ fromIon('{foo: [666, 1, 2]}').foo[0] }}
{# results in: '666' #}

{{ fromJson('{name: "John"}').name }}
{# results in: 'John' #}
```
...
The `kv` function will retrieve the value for the given key from the KV store.

```twig
{{ kv('my_key') }}
```

By default, it will search for the key within the same namespace as the flow. If the key is not found in the namespace, it will return the value as `null`.

In OSS, the key will be searched in the same namespace as the flow. However, in EE, you can specify the namespace which should be used to retrieve the value for the key. You can provide namespace as a parameter to the `kv` function.

```twig
{{ kv('my_key', 'company.team') }}
```

In the above example, the value for the key `my_key` will be retrieved from the `company.team` namespace.

You can also make the function throw an error if the given key is not found using `errorOnMissing` parameter.

```twig
{{ kv('my_key', errorOnMissing=true)}}
```
...
"{{ yaml('address: \"Paris, France\"') | jq('.address') | first | yaml }}"
{# results in: 'Paris, France' #}
```
