...
## Sript runners in Kestra 0.16.0 and beyond
...
Once you specify the `scriptRunner` type, you get the autocompletion and validation for the runner-specific properties. In the example above, the task allocates 1 CPU to the container.
...
In order to support `inputFiles`, `namespaceFiles`, and `outputFiles`, the `AwsBatchScriptRunner` currently relies on [multi-containers ECS jobs](https://docs.aws.amazon.com/batch/latest/userguide/multi-container-jobs.html) and creates three containers for each job:
