...
Related to the templated task, there are [new Pebble functions](../docs/07.concepts/expression/03.filter/yaml.md) to process YAML. You can now use the `yaml` and `indent` functions to parse and load YAML strings into objects that can be transformed using Pebble templating engine.

Thanks to [kriko](https://github.com/kriko) for [contributing this feature](https://github.com/kestra-io/kestra/pull/3283)!
...
You can now monitor your instance health and quickly identify any issues that need attention without having to rely on an additional observability tool.
