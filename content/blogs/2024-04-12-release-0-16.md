...
description: This release adds task runners allowing you to easily deploy your code to various remote environments, including Kubernetes, AWS Batch, Azure Batch, Google Batch, and more. We also introduce flow-level retries, dynamic tasks and a Cluster Health Dashboard.
...
We're thrilled to announce Kestra 0.16.0 which adds a new way to deploy your code to various remote environments, including a.o. Kubernetes, AWS Batch, Azure Batch, and Google Batch. We also introduce flow-level retries, new tasks, UI improvements, and more. This release makes it easier than ever to build, deploy, and manage your workflows. Let's dive into the highlights of this release.
...
Kestra 0.16.0 introduces a new `taskRunner` property that provides more flexibility than  `runner` and allows you to deploy your code to various remote environments, including [Kubernetes](https://develop.kestra.io/plugins/plugin-kubernetes/task-runners/runner/io.kestra.plugin.kubernetes.runner.KubernetesTaskRunner), [Docker](https://develop.kestra.io/plugins/plugin-docker/task-runners/runner/io.kestra.plugin.docker.runner.DockerTaskRunner),
 [AWS Batch](https://develop.kestra.io/plugins/plugin-aws/task-runners/runner/io.kestra.plugin.aws.runner.AwsBatchTaskRunner), [Azure Batch](https://develop.kestra.io/plugins/plugin-azure/task-runners/runner/io.kestra.plugin.azure.runner.AzureBatchTaskRunner), [Google Batch](https://develop.kestra.io/plugins/plugin-gcp/task-runners/runner/io.kestra.plugin.gcp.runner.GoogleBatchTaskRunner), and more coming in the future. Since each `taskRunner` type is a plugin, you can create your own (e.g. for Google CloudRun) and if you wish, you can contribute it to our open-source codebase.
...
This feature is particularly useful when you want to retry the entire flow if _any_ task fails. This way, you don't need to configure retries for each task individually.
...
Sometimes, you may want to programmatically enable or disable a trigger based on a certain condition. For example, when a business-critical process fails, you may want to automatically disable a trigger to prevent further executions until the issue is resolved. The [new](https://github.com/kestra-io/kestra/issues/2717) `Toggle` task allows you to do just that via a simple declarative task.
...
Related to the templated task, there are new Pebble functions to process YAML. You can now use the `yaml` and `indent` functions to parse and load YAML strings. This feature is particularly useful when you want to work with YAML data in your flows.
https://github.com/kestra-io/kestra/issues/3258
...
## The secret() function now returns null if the secret cannot be found

The `secret()` function now returns `null` if the secret cannot be found. This change makes it easier to handle missing secrets in your flows. A use case reported by our user was to fall back to an environment variable if a secret is not found. With [this change](https://github.com/kestra-io/kestra/issues/3162), you can now use the `secret()` function in combination with the [null-coalescing](https://kestra.io/docs/concepts/expression/operator#null-coalescing) operator `??` to use a default value if the secret is not found:
...
### Warning when editing an outdated revision
...
Thanks to this change, you will now also see the outputs of failed tasks (i.e. outputs generated up to the point of failure) in the Outputs tab, making troubleshooting easier.

### Better trigger display

We've improved the trigger display in the UI. Instead of only [showing the first letter](https://github.com/kestra-io/kestra/issues/2789), we now display a friendly emoji icon for each trigger type.

![better_trigger_display](/blogs/2024-04-12-release-0-16/better_trigger_display.png)

Disabled triggers are now greyed out in the topology view even when they are disabled only via an API call without changing the source code. This makes it easier to see which triggers are currently active.
...
You can now [authenticate](https://github.com/kestra-io/plugin-aws/issues/348) to AWS services using IAM Roles. This addition is particularly useful when you don't want to manage access keys e.g. on AWS EKS Kestra deployments.
...
We've added a (georgeous!) new cluster monitoring dashboard to the Enterprise Edition. This dashboard provides an overview of your instance health, including the status, configuration and metrics of all worker, executor, scheduler and webserver components. You can now monitor your instance health and quickly identify any issues that need attention without having to rely on an additional observability tool.
