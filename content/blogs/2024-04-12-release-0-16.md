...
Related to the templated task, there are [new Pebble functions](../docs/expressions/03.filter/yaml.md) to process YAML including the `yaml` and `indent` functions that allow you to parse and load YAML strings into objects. Those objects can then be further transformed using Pebble templating.
