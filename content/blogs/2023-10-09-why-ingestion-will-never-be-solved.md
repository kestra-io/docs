...
Another distinction compared to [Singer](https://www.singer.io/) is that Airbyte is primarily designed to be used from the UI, not via code. This brings us to [CloudQuery](https://www.cloudquery.io/) and [dltHub](https://dlthub.com/) which are code-centric and intended to be used by developers.
...
How does CloudQuery monetize? CloudQuery offers paid connectors for more advanced integrations, and there's also a plan for a [managed service](https://www.cloudquery.io/register-for-cloud) for running CloudQuery connectors in the cloud.
...
How does dltHub monetize? dlt plans to offer an [integration hub](https://www.cloudquery.io/register-for-cloud) which might resemble a connector marketplace and a managed service for data integration. It seems similar to CloudQuery's offering, but it's too early to tell how the two products will compare commercially.
...
As discussed before, data ingestion is a **moving target**. The source systems, schemas and their underlying APIs keep changing. Even though the existing and emerging tools can save a lot of time, none of them alone will solve the problem at its core as it's driven by competing commercial interests between systems to be integrated and the integration systems. SaaS vendors publishing their APIs are [not incentivized](https://matt-rickard.com/the-m-n-api-problem) to make this process easy for system integrators, as they would rather offer their own services to extract, analyze or integrate data for their customers. And when there's a need to introduce a breaking change to some APIs to gain a competitive edge, they'll likely do it without asking the data integration vendors for permission.

It's likely that emerging AI-powered integration tools might provide a more scalable solution in the near future, at least for building the connectors. Until then, we shouldn't complain either. Integrating data and systems is not as difficult as it used to be in the past. We have more options than ever for nearly every use case and target audience. And we have many great open-source communities focused on helping each other out, such as our [Slack](https://kestra.io/slack). Join the community and star us on [GitHub](https://github.com/kestra-io/kestra) to support the project.
