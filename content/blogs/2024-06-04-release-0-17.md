...
To use Real-time Triggers, you need to configure the trigger in your workflow YAML file. Here's an example of a Real-time Trigger listening to new messages in a Kafka topic and triggering exactly one workflow execution in real-time for each message received in a Kafka topic:
