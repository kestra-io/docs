---
title: Kestra 0.17.0 brings a lightning-fast Code Editor, Autocompletion, improved Git sync, and Real-time Event Triggers ⚡️
...
- Improved **naming** conventions for better consistency — a.o. `taskDefaults` are now `pluginDefaults` and the critical HTTP tasks are now part of the core plugin.
...
1. **Better Performance**: it loads lightning-fast ⚡️
...
With the release of Kestra 0.17.0, we are also introducing a [fully](New Git tasks https://github.com/kestra-io/plugin-git/issues/56) [redesigned](New Git tasks https://github.com/kestra-io/plugin-git/issues/57) Version Control integration, offering more flexibility as compared to the previous Git tasks. Here's a list of the new tasks:
- **PushFlows**: commit and push saved flows to a Git repository.
- **SyncFlows**: sync flows from a Git branch to one or more Kestra namespaces.
...
The `PushNamespaceFiles` and `SyncNamespaceFiles` tasks work in a similar way to the `PushFlows` and `SyncFlows` tasks, but for namespace files.

With these new tasks, you get more control over your Git workflows, making it easier to version control your flows and namespace files.
...
Kestra has a concept of [triggers](../docs/04.workflow-components/07.triggers.md) that can listen to external events and start a workflow execution when the event occurs. Most of these triggers poll external systems for status, which means that they check for new events at regular intervals e.g. every second. While this works well for most use cases, business-critical workflows require reacting to events as they happen with **millisecond latency**.
...
With this new feature, you can automate business-crucial processes in real-time. This covers scenarios such as fraud/anomaly detection, order processing, real-time predictions or recommendations, reacting to stock price changes, shipping and delivery notifications, and many more.

### How to Use Real-time Triggers

To use Real-time Triggers, you need to configure the trigger in your workflow YAML file. Here's an example of a Real-time Trigger listening to new messages in a Kafka topic and triggering exactly one workflow execution in real-time for each message received:
...
A video is worth a thousand words, so check out the new Getting Started experience in the video below:

TODO

### Improved Settings page
The Settings page has a [new structure](https://github.com/kestra-io/kestra/issues/1947) to make it easier to navigate and find the settings you need. The new structure groups settings into Theme Preferences, Date and Time Preferences as well as the Main Configuration settings.
...
### Renaming
We've done a lot of cleaning to ensure consistent and intuitive naming. Many core tasks, triggers and conditions have been renamed. We've also renamed `taskDefaults` to `pluginDefaults` to highlight that you can set default values for all plugins (_including triggers, task runners and more_), not just tasks.
...
Below, you can see an example of a vacation approval process that uses the Pause task to pause the workflow execution and wait for vacation approval from a manager. The user can approve or reject the request and provide a reason for their decision.
...
::alert{type="info"}
For more examples of how to use the Manual Approval pattern, check the [Pause and Resume](../docs/15.how-to-guides/parallel-vs-sequential.md) how-to guide.
::
...
The following example demonstrates a simple counter that loops until the counter reaches 10. The loop will run every 10 milliseconds until the counter reaches 10. The `Log` task will print the current iteration count in the logs.
...
tasks:
  - id: block_until_finished
    type: io.kestra.plugin.core.flow.WaitFor
    condition: "{{ outputs.poll.code != 200 }}" # replace with the actual condition e.g. {{ outputs.poll.body.status != 'finished' }}
    tasks:
      - id: poll
        type: io.kestra.plugin.core.http.Request
