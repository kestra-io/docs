...
🔹 Improved access to `taskrun.values` in `WorkingDirectory` tasks used as child tasks of other [flowable](https://kestra.io/docs/tutorial/flowable) tasks
...
### The problem that the new Editor solves

Kestra's approach to orchestration logic is centered around a human-readable YAML configuration, which allows for defining complex processes in a straightforward and declarative manner. With the Kestra UI and Editor, a few lines of YAML is all it takes to orchestrate any process or application, eliminating the need to manage environments or package code. This YAML configuration addresses:
...
- **Stay organized**: by combining tenants, namespaces and a nested folder structure in the embedded VS Code editor, you can organize your code and configuration files in a way that makes sense to you and your team.
...
In our ongoing effort to simplify workflow automation and streamline data processing at scale, we're thrilled to introduce a new task called `ForEachItem`. As the name suggests, this task allows you to iterate over a list of items and execute a subflow for each of them (_or for each batch containing multiple rows_).

### Problems that the `ForEachItem` task solves
...
Here's a quick look at the syntax for implementing the `ForEachItem` task:
...
```yaml
  - id: each
    ...
    inputs:
      your_subflow_input_name: "{{ taskrun.items }}" # items of the batch passed to the subflow's inputs
...
Here is the parent flow that uses the `ForEachItem` task to iterate over a list of items and run the `subflow` for each item (i.e. a batch split into `rows: 1` per iteration):
...
At the core of any robust workflow management system is the ability to manage and optimize resources. In line with this, we're introducing the flow-level `concurrency` setting. This flow-level property allows you to control the number of concurrent executions of a specific flow. By setting the `limit` key, you can define the maximum number of executions that can run simultaneously. This acts as a global concurrency limit for the particular flow, ensuring that the defined limit is respected across all executions, regardless of their initiation method — be it a trigger, webhook, API call, or an ad-hoc run from the UI.

---

### Problems solved by the flow-level concurrency setting

One of the primary challenges in workflow management is distributing work to ensure **optimal resource utilization**. This is particularly important in environments where resources are limited, and the number of concurrent executions needs to be controlled. For example, if your flow performs a resource-intensive operation, you may want to ensure that only a limited number of executions are running at any given time to avoid overloading the system. Often you also need to ensure that only a specific number of processes interact with a given database or API to limit the number of open database connections and avoid API request throttling. The concurrency limit allows you to queue additional executions above the allowed threshold to recuce the backpressure on the external system your flow is interacting with.
...
Tenants in Kestra can be managed in several ways. You can create and manage tenants via UI, using a CLI command, an API call, or using Kestra's Terraform provider.
...
The addition of multi-tenancy in Kestra represents a major milestone in addressing complex enterprise-grade orchestration requirements with support for resource and data isolation.

---

## New plugins (21 of them!)

Kestra 0.13 brings a broad array of new notification plugins to send alerts and notifications to your team when a flow completes or fails.
