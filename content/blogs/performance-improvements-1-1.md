...
- Any associated files in storage (e.g. logs archived to disk or object storage) are now deleted **asynchronously** on a separate thread pool. This means file I/O latency doesn’t hold up the database operations. The purge task triggers the file deletions and can immediately proceed, instead of waiting for each file to be removed.
