...
- **New UI features**
  - the Editor displays has been revamped, improving the visibility of the Editor options as well as the `Save` and `Actions` buttons
...
- **Other enhancements:**
  ...
  - new `ENUM` input type allowing you to ensure valid user inputs
...
Speaking of backfills, Kestra 0.15.0 [introduces](https://github.com/kestra-io/kestra/issues/2219) a new configuration allowing you to choose whether you want to automatically backfill missed schedules on server startup or not. This gives you more control over how your flows are executed, especially after a planned maintenance window or an outage. The `recoverMissedSchedules` configuration can be set to `ALL`, `NONE` or `LAST`, and can be defined globally or within a specific flow. For more information, check out the [Recover Missed Schedules](https://kestra.io/docs/workflow-components/triggers/schedule#recover-missed-schedules) documentation.
...
Lastly, the `Save` and `Actions` buttons have been moved for better visibility. Just look at the new Editor display!
