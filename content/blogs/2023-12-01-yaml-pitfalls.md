...
1. Kestra supports a concept of [subflows](https://kestra.io/plugins/core/tasks/flows/io.kestra.core.tasks.flows.subflow), which allows you to **treat each flow as a reusable module** that can be executed in any other flow. By using subflows, you can split a large workflow into smaller components that can be called and orchestrated from a parent flow in the same way as you would call a function.
