...
| **SQL Server Backend (Preview)** | Enterprise customers can now use SQL Server as a Kestra EE backend database. Note that this feature is in preview — we release it to get feedback from early adopters of this backend. For production JDBC deployment, we recommend Postgres at this time. | Enterprise Edition                  | [Link](/docs/enterprise/scim)                               |
...
Based on requests from several Enterprise customers, we have added SQL Server as a backend database option for Kestra EE. This feature is currently in preview, and we are looking for feedback from early adopters of this backend.

Check the Configuration Guide for setting up SQL Server as a backend database in [our documentation](/docs/configuration-guide/database).
