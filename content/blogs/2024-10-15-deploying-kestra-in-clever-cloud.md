...
[Clever Cloud](https://www.clever-cloud.com/) is a Platform as a Service provider that itself uses Kestra.
As they deploy Kestra in Clever Cloud, I was wondering how easy it is to effectively deploy Kestra in Clever Cloud, moreover I personally know some of the Clever Cloud developers and wanted to test their product since a long time ... so let's do it!

**Prerequisite**: you need a Clever Cloud account and the `clevercloud` CLI installed on your machine, see the [Clever Cloud quickstart](https://developers.clever-cloud.com/doc/quickstart/) for setup instructions if needed.
...
As Kestra is delivered as a Docker container, we will deploy it as a Docker application.
So the first thing is to create a Git repository and add a Dockerfile into it.

I'm using this Dockerfile to launch the latest version of Kestra in standalone more (all-in-one server mode):
...
Click on **NEXT**, you will then be able to set the application name and the location. Here we named it `kestra-clever` and deploy it in the France region.

![Step 2 - Docker](/blogs/2024-10-15-deploying-kestra-in-clever-cloud/clever-cloud-step-2-docker.png)

Click on **Finish**, you'll arrive to the add-ons page, we will create them later so click on **I don't need any add-ons**.

You will arrive at a page where you can configure environment variables, this is the most tricky part as you will need to configure properly Clever Cloud and Kestra to work together.
...
- `KESTRA_CONFIGURATION` to the Kestra [configuration](/docs/configuration) YAML file.

Here is the configuration file that I use, we will explain it in details.
...
5. As the Docker engine is not accessible from the Kestra container, we configure globally the `Process` task runner for all plugins via plugins default.

![Step 3 - Docker](/blogs/2024-10-15-deploying-kestra-in-clever-cloud/clever-cloud-step-3-docker.png)

Remember to click on **UPDATE CHANGES** before clicking on **NEXT** to save your changes. You will then arrive at a page which explains how to deploy a Docker application via git push, follow the instructions on this page and push your Git branch to Clever Cloud.
...
On the Clever Cloud console, click on **CREATE**, select **an add-on** then **Cellar S3 storage**.

![Step 6 - Cellar](/blogs/2024-10-15-deploying-kestra-in-clever-cloud/clever-cloud-step-6-cellar.png)

There is only one plan available, so click on **NEXT**, then click on **LINK** in front of the `kestra-clever` to link the add-on to the application. Linking the add-on will inject environment variables to the linked application with connection URL and credentials so it can be easily configured without needed to hardcode them. I would say this is a nice feature ;).

![Step 7 - Cellar](/blogs/2024-10-15-deploying-kestra-in-clever-cloud/clever-cloud-step-7-cellar.png)

Click on **NEXT**, then fill in the name of the Cellar bucket and change the location if you want. Here we create a bucket named `kestra-clever` in the Paris region.
...
On the Clever Cloud console, click on **CREATE**, select **an add-on** then **PostgreSQL**.

![Step 9 - PostgreSQL](/blogs/2024-10-15-deploying-kestra-in-clever-cloud/clever-cloud-step-9-postgres.png)

Select your plan, here I select **XXS Small Space** as it's a demo environment, but for a production environment you may choose a plan with more capacity. Click on **NEXT**, then click on **LINK** in front of the `kestra-clever` to link the add-on to the application.

![Step 10 - PostgreSQL](/blogs/2024-10-15-deploying-kestra-in-clever-cloud/clever-cloud-step-10-postgres.png)

Click on **NEXT**, then fill in the name of the PostgreSQL instance and change the version and location if you want. Here we create a PostgreSQL v15 instance named `kestra-clever` in the Paris region.
...
Now that everything has been created, go to your `kestra-clever` application and click on **START**.
...
Click on the link in the top right corner, Kestra should open in a new browser with a login popup!
