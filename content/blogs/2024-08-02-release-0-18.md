...
author:
  name: Anna Geller
  image: ageller
image: /blogs/2024-08-02-release-0-18.png
---
...
![namespaces_before_0_18](/blogs/2024-08-02-release-0-18/namespaces_before_0_18.png)
...
![namespaces_after_0_18](/blogs/2024-08-02-release-0-18/namespaces_after_0_18.png)
...
![namespace_oss](/blogs/2024-08-02-release-0-18/namespace_oss.png)

And here is how the same Namespace page looks like in the Enterprise Edition:

![namespace_ee](/blogs/2024-08-02-release-0-18/namespace_ee.png)
...
![audit_logs](/blogs/2024-08-02-release-0-18/audit_logs.jpg)
...
![audit_logs](/blogs/2024-08-02-release-0-18/audit_logs.gif)
...
Apart from the more **cost-effective handling of API calls** to secrets managers, we now also forward kestra-specific `tags` and (_for secrets managers that support it_) a `description` of the secret to the secrets manager.

![secrets_enhancements](/blogs/2024-08-02-release-0-18/secrets_enhancements.png)
...
Task Runners are plugins capable of executing your tasks in remote environments. We introduced task runners in Beta in Kestra 0.16.0, and since then, we've been working hard to improve their performance, stability, and usability. Thanks to the feedback from over 80 users and a large number of enhancements and bug fixes, Task Runners are now generally available and ready for production use at scale.

Here are some of the key enhancements to Task Runners added in this release:
...
Overall, these enhancements offer a more resilient file handling and provide more stability when recovering from failure in remote compute environments. Read more about the Task Runners GA in our dedicated [blog post](TODO).

![taskrunner_ga](/blogs/2024-08-02-release-0-18/taskrunner_ga.png)
...
![outputs](/blogs/2024-08-02-release-0-18/outputs.png)
...
## Realtime Triggers Enhancements 🚀
...
![realtime_trigger_ui](/blogs/2024-08-02-release-0-18/realtime_trigger_ui.png)
...
![temporal_default_logs](/blogs/2024-08-02-release-0-18/temporal_default_logs.png)

The GIF below shows how you can switch between both views:

![temporal_default_logs](/blogs/2024-08-02-release-0-18/temporal.gif)
...
![subflow_links](/blogs/2024-08-02-release-0-18/subflow_links.png)
...
![execute_curl](/blogs/2024-08-02-release-0-18/execute_curl.png)
...
1. the `json` filter to `jsonEncode` — it converts an object or data structure into a JSON string e.g. `{{ [1, 2, 3] | jsonEncode  }}`
2. the `json()` function to `jsonDecode` — it converts a JSON string into a data structure or object so you can access further attributes using a dot notation e.g. `{{ jsonDecode(kv('YOUR_JSON_KEY')).some_attribute }}`

We've also implemented equivalent functionality for ION — the new `ionDecode()` function converts an ION string into a data structure or object.  This function will raise an error if you try to parse a multi-line string (i.e. an ION file with multiple rows) for performance reasons.
...
![second](/blogs/2024-08-02-release-0-18/second.gif) TODO add GIF from notion
...
### New tasks to share and reuse code between namespaces
